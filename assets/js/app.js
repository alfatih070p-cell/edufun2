...
const StreakSystem = (() => {
  ...
  // Tunggu Firestore ready dulu (max 4 detik) supaya data user yang benar
  // sudah dimuat sebelum cek streak — mencegah duplicate check-in saat ganti akun
  function checkAndShow() {
    ...
    function _doCheck() {
      ...
      // Guard: jika lastShown sudah hari ini, langsung update UI saja, tidak ada aksi lain
      const alreadyShownToday = data.lastShown === todayStr;
      if (alreadyShownToday) {
        const sc = calcStreak(data.days);
        updateAllStreakUI(sc);
        return;
      }

      // Hari baru — tambahkan ke riwayat
      ...
      // Tandai sudah check-in hari ini SEBELUM kasih XP supaya tidak bisa duplikat
      data.lastShown = todayStr;
      saveData(data);  // simpan ke localStorage + Firestore

      updateAllStreakUI(streakCount);

      // Beri XP login bonus hanya jika benar-benar hari baru
    ...
    // Tunggu FirebaseSync selesai load data user dari Firestore
...
setTimeout(renderAllStatWidgets, 600);
...
(function() {
  ...
  // Saat login → load stat dari Firestore ke localStorage → render widget
  // FirebaseSync._applyStats sudah handle ini via onSnapshot,
  // tapi kita pastikan render widget juga dipanggil setelah data masuk
  const _origInit = typeof FirebaseSync !== 'undefined' ? FirebaseSync.init : null;
  if (typeof FirebaseSync !== 'undefined') {
    const __origInit = FirebaseSync.init;
    FirebaseSync.init = function() {
      __origInit.apply(this, arguments);
      // Setelah init, render widget dari localStorage (yang sudah diisi oleh _applyStats via onSnapshot)
      setTimeout(renderAllStatWidgets, 1500);
    };
  }
