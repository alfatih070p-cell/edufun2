...
function applyAc(hex) {
  ...
  // Refresh calendar ring agar mengikuti warna tema baru
  setTimeout(() => {
    if (typeof AgendaSystem !== 'undefined' && AgendaSystem.refresh) AgendaSystem.refresh();
    if (typeof agHomeCalRender === 'function') agHomeCalRender();
  }, 0);
}
...
function playMeow() {
  try {
    ...
    // Meow: oscillator frekuensi naik lalu turun
    const osc = ctx.createOscillator();
    const gain = ctx.createGain();
    osc.connect(gain);
...
const AgendaSystem = (() => {
  ...
  // Build multi-color ring style for calendar day cells
  // Build SVG ring overlay for calendar day cells using accent color
  ...
  function renderDayEvents() {
    ...
    const _cp = { red: 0, yellow: 1, green: 2 };
    const events = (agendaDB[selectedDate] || []).sort((a,b) => {
      const pa = _cp[a.color] ?? 99, pb = _cp[b.color] ?? 99;
    ...
    if (doneEvents.length) {
      ...
      const totalXP = unclaimedCount * 2;
      const totalCoins = unclaimedCount * 1;
      const rewardBadge = unclaimedCount > 0
        ...
        : '';
      const collectAllBtn = unclaimedCount > 1
        ? `<button onclick="event.stopPropagation();AgendaSystem.claimAllRewards('${selectedDate}')"
            class="ag-collect-all-btn"
            style="margin-top:.55rem;width:100%;padding:.45rem .8rem;background:linear-gradient(135deg,#f59e0b,#f97316);color:#fff;border:none;border-radius:10px;font-weight:800;font-size:.82rem;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:.4rem;transition:opacity .15s"
            onmouseover="this.style.opacity='.85'" onmouseout="this.style.opacity='1'">
            🎁 Klaim Semua &nbsp;·&nbsp; <span style="font-size:.75rem;opacity:.92">+${totalXP} XP &amp; +${totalCoins} Koin</span>
          </button>`
        : '';
  ...
  function renderHomeAgenda() {
    ...
    if (!list) return;
    const today = todayStr();
    const _cp2 = { red: 0, yellow: 1, green: 2 };
    const events = (agendaDB[today] || []).sort((a,b) => {
      const pa = _cp2[a.color] ?? 99, pb = _cp2[b.color] ?? 99;
      if (pa !== pb) return pa - pb;
      return a.timeStart.localeCompare(b.timeStart);
    });
    if (!events.length) {
    ...
    const nowHM2 = (() => { const n=new Date(); return n.getHours()*60+n.getMinutes(); })();
    const today2 = (() => { const d=new Date(); const p=n=>String(n).padStart(2,'0'); return `${d.getFullYear()}-${p(d.getMonth()+1)}-${p(d.getDate())}`; })();
    const unclaimedHome = events.filter(ev => {
      if (ev.claimed) return false;
      const [eh,em] = (ev.timeEnd||'00:00').split(':').map(Number);
      return nowHM2 >= eh*60+em;
    });
    const collectHomeTotalXP = unclaimedHome.length * 2;
    const collectHomeTotalCoins = unclaimedHome.length;
    const collectHomeBtn = unclaimedHome.length > 0
      ? `<button onclick="AgendaSystem.claimAllRewards('${today2}')"
          style="margin-top:.5rem;width:100%;padding:.38rem .7rem;background:linear-gradient(135deg,#f59e0b,#f97316);color:#fff;border:none;border-radius:8px;font-weight:800;font-size:.78rem;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:.35rem;box-shadow:0 2px 6px #f9731666;transition:opacity .15s"
          onmouseover="this.style.opacity='.85'" onmouseout="this.style.opacity='1'">
          🎁 Klaim ${unclaimedHome.length} Selesai &nbsp;·&nbsp; <span style="opacity:.9;font-size:.72rem">+${collectHomeTotalXP}XP +${collectHomeTotalCoins}🪙</span>
        </button>`
      : '';
    list.innerHTML = events.slice(0, 3).map(ev => {
      const [eh,em] = (ev.timeEnd||'00:00').split(':').map(Number);
      const isDone = nowHM2 >= eh*60+em;
      return `<div style="display:flex;align-items:center;gap:.55rem;padding:.4rem 0;border-bottom:1px solid #f1f5f9;${isDone && ev.claimed ? 'opacity:.45' : ''}">
        ...
        <div style="flex:1;min-width:0">
          <div style="font-weight:700;font-size:.82rem;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;${isDone && ev.claimed ? 'text-decoration:line-through;color:#94a3b8' : ''}">${ev.name}</div>
          <div style="font-size:.72rem;color:#94a3b8">${ev.timeStart} – ${ev.timeEnd}${isDone && !ev.claimed ? ' · <span style="color:#f97316;font-weight:700">belum diklaim</span>' : ''}</div>
        </div>
        ${isDone && !ev.claimed ? '<span style="font-size:.8rem">🎁</span>' : isDone ? '<span style="font-size:.75rem">✅</span>' : ''}
      </div>`;
    }).join('') + (events.length > 3 ? `<div style="font-size:.74rem;color:var(--ac);text-align:right;margin-top:.3rem;font-weight:700">+${events.length-3} lagi</div>` : '') + collectHomeBtn;
  }
  ...
  function openForm(dateStr, eventId) {
    ...
    const colorStyles = { red: { border:'#ef4444',bg:'#fee2e2',color:'#991b1b' }, yellow: { border:'#eab308',bg:'#fef9c3',color:'#854d0e' }, green: { border:'#22c55e',bg:'#dcfce7',color:'#166634' } };
    ...
    if (eventId) {
      ...
      if (ev) {
        ...
        if (cb) { cb.classList.add('sel'); const s=colorStyles[ev.color]; if(s){cb.style.borderColor=s.border;cb.style.background=s.bg;cb.style.color=s.color;} }
      }
    } else {
  ...
  return {
    ...
    claimAllRewards: (ds) => {
      ...
      evArr.forEach(ev => {
        ...
        const isPast = (ds < todayCheck) || (ds === todayCheck && nowHM >= endMin);
        if (!isPast) return;
        ev.claimed = true;
        count++;
      });
      ...
      const totalCoins = count * 1;
      if (typeof addXP === 'function') addXP(totalXP);
      if (typeof addCoins === 'function') addCoins(totalCoins);
      if (typeof SFX !== 'undefined' && SFX.levelUp) SFX.levelUp();
      else if (typeof SFX !== 'undefined') SFX.correct();
      if (typeof toast === 'function') toast('🎉 ' + count + ' agenda diklaim! +' + totalXP + ' XP & +' + totalCoins + ' Koin!');
      renderDayEvents();
    },
    setColor: (c) => { selectedColor = c; },
    getSelectedDate: () => selectedDate,
    refresh: () => { renderCalendar(); renderDayEvents(); renderHomeAgenda(); setTimeout(agHomeCalRender, 20); },
...
function selectAgColor(color, btn) {
  ...
  btn.classList.add('sel');
  // Apply active style based on color
  const colorStyles = {
  ...
  if (s) { btn.style.borderColor = s.border; btn.style.background = s.bg; btn.style.color = s.color; }
}
