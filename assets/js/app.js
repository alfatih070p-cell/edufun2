...
// Accent diterapkan oleh FirebaseSync setelah data user dimuat dari Firestore.
// Default sementara sampai Firestore reply:
...
function renderAv(svgId) {
  ...
  let bottom = '';
  if (bs === 'pants') bottom = `<rect x="38" y="138" width="20" height="38" rx="6" fill="${bc}"/><rect x="62" y="138" width="20" height="38" rx="6" fill="${bc}"/>`;
  else if (bs === 'shorts') bottom = `<rect x="38" y="138" width="20" height="22" rx="6" fill="${bc}"/><rect x="62" y="138" width="20" height="22" rx="6" fill="${bc}"/>`;
  else if (bs === 'skirt') bottom = `<path d="M36,138 Q40,176 60,176 Q80,176 84,138 Z" fill="${bc}"/>`;
  else if (bs === 'longskirt') bottom = `<path d="M36,138 Q32,182 60,182 Q88,182 84,138 Z" fill="${bc}"/>`;
  svg.innerHTML = `
    ${hair}
...
function renderAvWithData(svgId, data) {
  ...
  let dbottom = '';
  if (dbs === 'pants') dbottom = `<rect x="38" y="138" width="20" height="38" rx="6" fill="${dbc}"/><rect x="62" y="138" width="20" height="38" rx="6" fill="${dbc}"/>`;
  else if (dbs === 'shorts') dbottom = `<rect x="38" y="138" width="20" height="22" rx="6" fill="${dbc}"/><rect x="62" y="138" width="20" height="22" rx="6" fill="${dbc}"/>`;
  else if (dbs === 'skirt') dbottom = `<path d="M36,138 Q40,176 60,176 Q80,176 84,138 Z" fill="${dbc}"/>`;
  else if (dbs === 'longskirt') dbottom = `<path d="M36,138 Q32,182 60,182 Q88,182 84,138 Z" fill="${dbc}"/>`;
  svg.innerHTML = `
    ${hair}
    <ellipse cx="60" cy="52" rx="22" ry="26" fill="${skin}"/>
    <ellipse cx="50" cy="55" rx="6" ry="7" fill="${skin}" opacity=".35"/>
...
function openBab(subj, bab) {
  const babPages = {
    ...
    'Fluida Statis (Tekanan & Archimedes)': '../pages/11/fisika/fluida-statis.html',
    'Fluida Dinamis (Bernoulli)': '../pages/11/fisika/fluida-dinamis.html',
    'Suhu & Kalor': '../pages/11/fisika/suhu-kalor.html',
    'Perpindahan Kalor': '../pages/11/fisika/perpindahan-kalor.html',
    'Teori Kinetik Gas': '../pages/11/fisika/teori-kinetik-gas.html',
    'Gelombang Mekanik': '../pages/11/fisika/gelombang-mekanik.html',
    'Gelombang Bunyi': '../pages/11/fisika/gelombang-bunyi.html',
    'Optik Geometri (Cermin & Lensa)': '../pages/11/fisika/optik-geometri.html',
    // Kelas 11 Matematika
    'Fungsi & Fungsi Invers': '../pages/11/matematika/fungsi-invers.html',
    'Komposisi Fungsi': '../pages/11/matematika/komposisi-fungsi.html',
    'Matriks & Operasinya': '../pages/11/matematika/matriks-operasi.html',
    'Determinan & Invers Matriks': '../pages/11/matematika/determinan-invers.html',
    'Vektor di Bidang & Ruang': '../pages/11/matematika/vektor.html',
    'Transformasi Geometri': '../pages/11/matematika/transformasi-geometri.html',
    'Barisan & Deret Aritmetika': '../pages/11/matematika/barisan-aritmetika.html',
    'Barisan & Deret Geometri': '../pages/11/matematika/barisan-geometri.html',
    'Limit Fungsi Aljabar': '../pages/11/matematika/limit-fungsi.html',
  };
  const page = babPages[bab];
  if (page) {
    // Catat aktivitas belajar
    logActivity(subj, 15, 'materi');
    // Tunggu sebentar agar Firestore write selesai sebelum navigasi
    // (debounce onMateriOpen adalah 300ms, kasih buffer 600ms)
    setTimeout(() => { window.location.href = page; }, 650);
  }
}
...
(function initFromSession() {
  ...
  xp       = session.xp      || 0;
  coins    = 0; // akan diisi oleh FirebaseSync dari Firestore
  ...
  // Streak dari session (Firestore akan update via FirebaseSync setelah pull)
  streak = session.streak || 0;
...
(function() {
  ...
  // Override materiStatRecord → update memori + push ke Firestore
  ...
  window.materiStatRecord = function(mapel, bab) {
    ...
    // Push ke Firestore — debounced di onMateriOpen
...
const SHOP_ITEMS = {
  skin: [
    ...
    { id:'skin-5', name:'Fox Abu',        price:35  },
    { id:'skin-6', name:'Fox Oren',       price:35  },
  ],
  acc: [
...
function _saveShopToFirestore() {
  if (typeof FirebaseSync !== 'undefined') {
    FirebaseSync.saveShop({
      owned:      window._shopOwned,
      activeSkin: window._shopActiveSkin,
...
function applyMascotSkin() {
  ...
  const skins = {
    'skin-1': `<ellipse cx="40" cy="52" rx="18" ry="22" fill="#3b82f6"/><circle cx="40" cy="28" r="18" fill="#FDDBB4"/><circle cx="34" cy="26" r="3.5" fill="#fff"/><circle cx="46" cy="26" r="3.5" fill="#fff"/><circle cx="35" cy="27" r="2" fill="#1e293b" id="m-eye-l"/><circle cx="47" cy="27" r="2" fill="#1e293b" id="m-eye-r"/><circle cx="35.8" cy="26.2" r=".7" fill="#fff"/><circle cx="47.8" cy="26.2" r=".7" fill="#fff"/><path id="m-mouth" d="M34,34 Q40,39 46,34" stroke="#b45309" stroke-width="2" fill="none" stroke-linecap="round"/><ellipse cx="28" cy="31" rx="5" ry="3" fill="#fca5a5" opacity=".5"/><ellipse cx="52" cy="31" rx="5" ry="3" fill="#fca5a5" opacity=".5"/><ellipse cx="22" cy="53" rx="5" ry="11" fill="#FDDBB4" transform="rotate(-15 22 53)"/><ellipse cx="58" cy="53" rx="5" ry="11" fill="#FDDBB4" transform="rotate(15 58 53)"/><rect x="26" y="62" width="28" height="18" rx="4" fill="#fff" stroke="#3b82f6" stroke-width="1.5"/><line x1="40" y1="63" x2="40" y2="79" stroke="#3b82f6" stroke-width="1.5"/><rect x="24" y="12" width="32" height="5" rx="2.5" fill="#1e40af"/><rect x="30" y="6" width="20" height="10" rx="3" fill="#1e40af"/><rect x="36" y="5" width="8" height="3" rx="1.5" fill="#fbbf24"/>`,
    'skin-2': `<ellipse cx="40" cy="52" rx="18" ry="22" fill="#7c3aed"/><circle cx="40" cy="28" r="18" fill="#C68642"/><circle cx="34" cy="26" r="3.5" fill="#fff"/><circle cx="46" cy="26" r="3.5" fill="#fff"/><circle cx="35" cy="27" r="2" fill="#1e293b" id="m-eye-l"/><circle cx="47" cy="27" r="2" fill="#1e293b" id="m-eye-r"/><path id="m-mouth" d="M34,34 Q40,39 46,34" stroke="#92400e" stroke-width="2" fill="none" stroke-linecap="round"/><ellipse cx="22" cy="53" rx="5" ry="11" fill="#C68642" transform="rotate(-15 22 53)"/><ellipse cx="58" cy="53" rx="5" ry="11" fill="#C68642" transform="rotate(15 58 53)"/><rect x="26" y="62" width="28" height="18" rx="4" fill="#fff" stroke="#7c3aed" stroke-width="1.5"/><line x1="40" y1="63" x2="40" y2="79" stroke="#7c3aed" stroke-width="1.5"/><rect x="24" y="12" width="32" height="5" rx="2.5" fill="#5b21b6"/><rect x="30" y="6" width="20" height="10" rx="3" fill="#5b21b6"/>`,
    'skin-3': `<path d="M68,90 Q88,75 82,55 Q85,60 80,70 Q86,65 76,82 Z" fill="#b0b8c1"/><ellipse cx="44" cy="84" rx="28" ry="22" fill="#c8cdd2"/><ellipse cx="44" cy="86" rx="18" ry="14" fill="#f0f0f0"/><path d="M20,38 L26,18 L36,38 Z" fill="#c8cdd2"/><path d="M52,38 L62,18 L68,38 Z" fill="#c8cdd2"/><circle cx="44" cy="46" r="26" fill="#c8cdd2"/><ellipse cx="44" cy="53" rx="13" ry="9" fill="#f0f0f0"/><circle cx="35" cy="44" r="7" fill="#fff"/><circle cx="35" cy="44" r="3.5" fill="#111" id="m-eye-l"/><circle cx="33.5" cy="42.5" r="1.4" fill="#fff"/><circle cx="53" cy="44" r="7" fill="#fff"/><circle cx="53" cy="44" r="3.5" fill="#111" id="m-eye-r"/><circle cx="51.5" cy="42.5" r="1.4" fill="#fff"/><path d="M42,52 L44,54.5 L46,52 Q44,51 42,52 Z" fill="#e88fa0"/><path id="m-mouth" d="M40,55 Q44,58.5 48,55" stroke="#2d2d2d" stroke-width="1.5" fill="none"/>`,
    'skin-4': `<path d="M68,90 Q88,75 82,55 Q85,60 80,70 Q86,65 76,82 Z" fill="#d97706"/><ellipse cx="44" cy="84" rx="28" ry="22" fill="#e8a04a"/><ellipse cx="44" cy="86" rx="18" ry="14" fill="#fef3c7"/><path d="M20,38 L26,18 L36,38 Z" fill="#e8a04a"/><path d="M52,38 L62,18 L68,38 Z" fill="#e8a04a"/><circle cx="44" cy="46" r="26" fill="#e8a04a"/><ellipse cx="44" cy="53" rx="13" ry="9" fill="#fef3c7"/><circle cx="35" cy="44" r="7" fill="#fff"/><circle cx="35" cy="44" r="3.5" fill="#111" id="m-eye-l"/><circle cx="53" cy="44" r="7" fill="#fff"/><circle cx="53" cy="44" r="3.5" fill="#111" id="m-eye-r"/><path id="m-mouth" d="M40,55 Q44,58.5 48,55" stroke="#2d2d2d" stroke-width="1.5" fill="none"/>`,
    'skin-5': `<path d="M58,50 Q90,38 86,12 Q88,20 84,30 Q92,20 84,38 Q86,48 62,54 Z" fill="#e8edf2"/><ellipse cx="40" cy="58" rx="28" ry="24" fill="#f0f4f8"/><ellipse cx="40" cy="62" rx="18" ry="17" fill="#fafcff"/><path d="M26,14 Q20,-4 34,4 Q32,10 30,14 Z" fill="#e8edf2"/><path d="M54,12 Q56,-6 68,4 Q64,10 62,14 Z" fill="#e8edf2"/><circle cx="40" cy="28" r="22" fill="#f0f4f8"/><ellipse cx="39" cy="36" rx="11" ry="8" fill="#fafcff"/><circle cx="32" cy="26" r="3" fill="#1a0a04" id="m-eye-l"/><circle cx="30.8" cy="24.8" r="1.1" fill="#fff"/><circle cx="48" cy="26" r="3" fill="#1a0a04" id="m-eye-r"/><circle cx="46.8" cy="24.8" r="1.1" fill="#fff"/><ellipse cx="39" cy="34" rx="2" ry="1.5" fill="#1a0a04"/><path id="m-mouth" d="M35,36.5 Q39,39 43,36.5" stroke="#2d1a0e" stroke-width="1.3" fill="none"/>`,
    'skin-6': `<path d="M58,50 Q90,38 86,12 Q88,20 84,30 Q92,20 84,38 Q86,48 62,54 Z" fill="#7a2a08"/><ellipse cx="40" cy="58" rx="28" ry="24" fill="#b34a10"/><ellipse cx="40" cy="62" rx="18" ry="17" fill="#f5ede0"/><path d="M26,14 Q20,-4 34,4 Q32,10 30,14 Z" fill="#b34a10"/><path d="M54,12 Q56,-6 68,4 Q64,10 62,14 Z" fill="#b34a10"/><circle cx="40" cy="28" r="22" fill="#b34a10"/><ellipse cx="39" cy="36" rx="12" ry="9" fill="#f5ede0"/><circle cx="32" cy="26" r="3" fill="#1a0a04" id="m-eye-l"/><circle cx="48" cy="26" r="3" fill="#1a0a04" id="m-eye-r"/><ellipse cx="39" cy="34" rx="2" ry="1.5" fill="#1a0a04"/><path id="m-mouth" d="M35,36.5 Q39,39 43,36.5" stroke="#2d1a0e" stroke-width="1.3" fill="none"/>`
  };
