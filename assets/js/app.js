...
function renderMateriStatWidget() {
  ...
  const elLastBadge = document.getElementById('materi-stat-last-badge');
  ...
  if (total > 0 && d.lastMapel) {
    ...
    const mapelLabel = mapelShort[d.lastMapel] || d.lastMapel;
    if (elMapel) elMapel.textContent = mapelLabel;
    if (elLastBadge && d.lastTs) elLastBadge.textContent = _timeAgo(d.lastTs);
...
// Render awal: tampilkan data yang sudah ada di localStorage (dari sesi sebelumnya)
// sambil menunggu Firestore pull selesai. Ini mencegah widget tampak kosong.
setTimeout(function() {
  // Cek apakah ada data apapun di localStorage sebelum render
  try {
    const hasArena  = localStorage.getItem('ef_stat_arena');
    const hasMateri = localStorage.getItem('ef_stat_materi');
    const hasPomo   = localStorage.getItem('ef_stat_pomo');
    if (hasArena || hasMateri || hasPomo) {
      renderAllStatWidgets(); // render dari cache localStorage dulu
    }
  } catch(e) {}
}, 300);
// FirebaseSync._applyStatsToApp akan render ulang setelah data Firestore masuk (overwrite cache)

// Render ulang saat buka panel home (data sudah ada di localStorage dari Firestore pull)
(function() {
...
(function() {
  ...
  // Override arenaStatRecord → update localStorage + push ke Firestore
  const _origArena = window.arenaStatRecord;
  window.arenaStatRecord = function(won) {
    if (_origArena) _origArena(won); // update localStorage
  ...
  // Override materiStatRecord → update localStorage + push ke Firestore
  const _origMateri = window.materiStatRecord;
  window.materiStatRecord = function(mapel, bab) {
    if (_origMateri) _origMateri(mapel, bab); // update localStorage
  ...
  // pomoStatRecord hanya update localStorage (untuk tampilan widget)
  ...
  window.pomoStatRecord = function(durationMinutes) {
    if (_origPomo) _origPomo(durationMinutes); // hanya update localStorage
