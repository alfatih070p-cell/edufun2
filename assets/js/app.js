...
// TIDAK ada setTimeout render di sini.
// renderAllStatWidgets dipanggil oleh FirebaseSync._applyStatsToApp
// setelah data benar-benar datang dari Firestore.
...
(function() {
  ...
  // Override arenaStatRecord → update localStorage stat lokal saja
  // (onBattleResult ke Firestore sudah dipanggil langsung dari finishBattle)
  const _origArena = window.arenaStatRecord;
  window.arenaStatRecord = function(won) {
    if (_origArena) _origArena(won);
    // TIDAK panggil FirebaseSync.onBattleResult di sini
    // karena finishBattle() sudah memanggil FirebaseSync.onBattleResult() langsung
  };

  // Override materiStatRecord → push ke Firestore
  ...
  // Override pomoStatRecord → push ke Firestore
