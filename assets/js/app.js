...
// ===========================
function addXP(amt) {
  // Log XP change for history chart
  try {
...
function getRooms() {
  try { return JSON.parse(localStorage.getItem('edufun_rooms') || '{}'); } catch(e) { return {}; }
}
function saveRooms(rooms) {
  localStorage.setItem('edufun_rooms', JSON.stringify(rooms));
}
function cleanExpiredRooms() {
  ...
  Object.keys(rooms).forEach(code => {
    const r = rooms[code];
    // Auto-delete if: all players left, or room is older than 2 hours
    const allLeft = !r.players || Object.values(r.players).every(p => p.left);
    const tooOld = now - r.created > 2 * 60 * 60 * 1000;
    if (allLeft || tooOld) delete rooms[code];
  });
  saveRooms(rooms);
}
...
function generatePrivateRoom() {
  ...
  rooms[code] = {
    code, mapel: brMapel, level: brLevel, maxPlayers: brSize,
    created: Date.now(),
    players: { [playerId]: { name: playerId, joined: Date.now(), left: false, ready: false } }
  };
  saveRooms(rooms);
  brCode = code;
...
function joinPrivateRoom() {
  ...
  cleanExpiredRooms();
  const rooms = getRooms();
  if (!rooms[code]) { statusEl.textContent = '❌ Kode tidak ditemukan atau room sudah dihapus'; statusEl.style.color = '#ef4444'; return; }
  const room = rooms[code];
  const players = Object.values(room.players).filter(p => !p.left);
  if (players.length >= room.maxPlayers) { statusEl.textContent = '❌ Room sudah penuh (' + room.maxPlayers + '/' + room.maxPlayers + ')'; statusEl.style.color = '#ef4444'; return; }
  const playerId = userName || 'Pemain';
  room.players[playerId + '_' + Date.now()] = { name: playerId, joined: Date.now(), left: false, ready: false };
  rooms[code] = room;
  saveRooms(rooms);
  brCode = code;
...
function openPrivateLobby(code) {
  ...
  document.getElementById('priv-room-meta').textContent = room.mapel + ' · ' + room.level + ' · Maks. ' + room.maxPlayers + ' pemain';
  showArenaMode('private-lobby');
  renderPrivatePlayers(code);
  // Start polling for new players
  if (brPollIv) clearInterval(brPollIv);
  brPollIv = setInterval(() => {
    cleanExpiredRooms();
    const rooms2 = getRooms();
    if (!rooms2[code]) { clearInterval(brPollIv); toast('⚠️ Room sudah dihapus'); showArenaMode('sel'); return; }
    renderPrivatePlayers(code);
  }, 2000);
}

function renderPrivatePlayers(code) {
  const rooms = getRooms();
  const room = rooms[code];
  if (!room) return;
  const players = Object.values(room.players).filter(p => !p.left);
  ...
  list.innerHTML = players.map((p, i) => `
    ...
    </div>`).join('') + (players.length < room.maxPlayers ? `<div style="text-align:center;padding:.5rem;color:#94a3b8;font-size:.76rem;border:1.5px dashed #e2e8f0;border-radius:10px">⏳ Menunggu ${room.maxPlayers - players.length} pemain lagi...</div>` : '');
...
function leavePrivateRoom() {
  ...
  if (brCode) {
    const rooms = getRooms();
    const room = rooms[brCode];
    if (room) {
      const playerId = userName || 'Pemain';
      Object.keys(room.players).forEach(k => { if (room.players[k].name === playerId) room.players[k].left = true; });
      // Check if all left → delete room
      const anyActive = Object.values(room.players).some(p => !p.left);
      if (!anyActive) delete rooms[brCode];
      else rooms[brCode] = room;
      saveRooms(rooms);
    }
    brCode = '';
  }
...
function startPrivateBattle() {
  ...
  if (!room) { toast('❌ Room tidak ditemukan'); showArenaMode('sel'); return; }
  const players = Object.values(room.players).filter(p => !p.left);
  const opponent = players.find(p => p.name !== (userName || 'Pemain'));
...
function setStatScale(scale, el) {
  ...
  el.style.color = '#fff';
  document.getElementById('stat-scale-label').textContent = statLabels[scale];
...
const OnlineLeaderboard = (() => {
  ...
  function render() {
    ...
    // Sort by XP descending, exclude current user from list (shown in footer)
    const sorted = [...realPlayers].sort((a, b) => b.xp - a.xp).slice(0, 8);

    rows.innerHTML = sorted.map((p, i) => `
      <div style="display:flex;align-items:center;gap:.65rem;padding:.65rem 1rem;border-bottom:1px solid #f8fafc">
        <div style="width:22px;font-size:.95rem">${rankMedals[i] || (i+1)+'.'}</div>
        <div style="flex:1">
        ...
        <div style="width:55px;text-align:right;font-weight:700;font-size:.85rem">${p.xp} XP</div>
      </div>`).join('');

    // My rank position
    const myRankEl = document.getElementById('lb-my-rank');
    if (myRankEl) {
      const myPos = sorted.filter(p => p.xp > xp).length + 1;
      myRankEl.textContent = 'Peringkat #' + myPos;
    }
    const myWrEl = document.getElementById('lb-my-wr');
    if (myWrEl) myWrEl.textContent = getRankLabel(xp);
  }

  function loadFromFirebase() {
    // Try to read leaderboard from Firebase if available
    try {
      if (typeof firebase !== 'undefined' && firebase.database) {
        firebase.database().ref('users').orderByChild('xp').limitToLast(20).once('value', snap => {
          realPlayers = [];
          snap.forEach(child => {
            const d = child.val();
            if (d && d.displayName) {
              realPlayers.push({ name: d.displayName, xp: d.xp || 0, school: d.school || '' });
            }
          });
          render();
        });
      } else if (typeof FirebaseSync !== 'undefined' && FirebaseSync.getLeaderboard) {
        FirebaseSync.getLeaderboard(players => {
          realPlayers = players || [];
          render();
        });
      } else {
