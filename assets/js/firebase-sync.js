...
const FirebaseSync = (() => {
  ...
  function init() {
    ...
    // Guard: jangan double-init untuk UID yang sama
  ...
  async function _pullAllFromFirestore() {
    ...
    try {
      ...
      if (!doc.exists) {
        ...
        _ready = true;
        // Render widget dengan data kosong supaya tetap tampil (nilai 0)
        setTimeout(() => {
      ...
      _ready = true;
      // Tetap render widget (data kosong) agar tidak stuck
      setTimeout(() => {
        if (typeof renderAllStatWidgets === 'function') renderAllStatWidgets();
      }, 200);
    }
  ...
  function _applyStatsToApp(stats) {
    ...
    // ── Tulis ke localStorage — SELALU sebelum render ──
    console.log('[FirebaseSync] Menulis stat ke localStorage dan merender widget...');
    // Arena stat
    const arenaData = {
    ...
    console.log('[FirebaseSync] arena data dari Firestore:', arenaData);

    // Materi stat
    const materiData = {
      total:     Number(stats.materiTotal)  || 0,
    ...
    console.log('[FirebaseSync] materi data dari Firestore:', materiData);
    ...
    console.log('[FirebaseSync] pomo data dari Firestore:', pomoData);

    // ── Render semua widget stat ──
    // Gunakan setTimeout agar dipastikan berjalan setelah seluruh app.js selesai dimuat
    function _tryRenderWidgets(attempts) {
      if (typeof renderArenaStatWidget === 'function' &&
        ...
        console.warn('[FirebaseSync] Stat widget functions tidak ditemukan setelah retry');
      }
    }
  ...
  function stop() {
    ...
    _uid        = null;
    _ready      = false;
    _initCalled = false; // reset agar relogin bisa init ulang
