...
const FirebaseSync = (() => {
  ...
  function _applyToApp(data) {
    ...
    if (data.accent != null) {
      ...
      // Terapkan warna tanpa trigger saveAccent (hindari loop onSnapshot)
      if (typeof _applyAccentOnly === 'function') _applyAccentOnly(data.accent);
      else if (typeof applyAc === 'function') applyAc(data.accent);
    } else if (_isNewUid) {
      localStorage.removeItem('ef_ac');
      if (typeof _applyAccentOnly === 'function') _applyAccentOnly('#3b82f6');
      else if (typeof applyAc === 'function') applyAc('#3b82f6');
    }
    ...
    // Selalu panggil _applyStats: jika stats null/kosong, widget di-render dengan nilai default
    _applyStats(data.stats || {});
  ...
  function _applyStats(stats) {
    ...
    if (stats.battleTotal != null || stats.battleWins != null || 'battleTotal' in stats || stats.battleTotal === 0) {
      ...
      // Tidak ada data battle sama sekali di Firestore → render widget dengan nilai default
      if (!localStorage.getItem('ef_stat_arena')) {
        localStorage.setItem('ef_stat_arena', JSON.stringify({ total: 0, wins: 0, lastTs: null, lastMapel: null }));
      }
      if (typeof renderArenaStatWidget === 'function') renderArenaStatWidget();
    }

    if (stats.materiTotal != null || stats.lastMateriTs != null || 'materiTotal' in stats) {
      ...
      // Tidak ada data materi di Firestore → set default jika belum ada
      if (!localStorage.getItem('ef_stat_materi')) {
        localStorage.setItem('ef_stat_materi', JSON.stringify({ total: 0, lastMapel: null, lastBab: null, lastTs: null }));
      }
      if (typeof renderMateriStatWidget === 'function') renderMateriStatWidget();
    }
