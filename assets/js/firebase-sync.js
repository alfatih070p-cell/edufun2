...
const FirebaseSync = (() => {
  ...
  function _applyToApp(data) {
    ...
    // Agenda — SELALU overwrite dari Firestore (bukan merge)
    // Kalau user baru belum punya agenda → set kosong {} supaya data user lama tidak tersisa
    const agendaData = (data.agenda != null && typeof data.agenda === 'object') ? data.agenda : {};
    ...
    // Streak — SELALU overwrite dari Firestore (termasuk lastShown untuk anti-duplikat check-in)
    const streakData = (data.streak != null && typeof data.streak === 'object')
      ? data.streak
      : { days: [], lastShown: null };
    // Pastikan lastShown selalu ada supaya checkAndShow bisa cek dengan benar
    ...
    if (data.accent != null) {
      // Simpan ke localStorage dulu supaya sinkron dengan applyAc
      localStorage.setItem('ef_ac', data.accent);
      if (typeof applyAc === 'function') applyAc(data.accent);
    } else {
      // User baru belum punya accent — reset ke default
    ...
    // Selalu overwrite (termasuk clear jika user baru belum punya data)
    localStorage.setItem('ef_xp_history',
      JSON.stringify(Array.isArray(data.xpHistory) ? data.xpHistory : []));
    localStorage.setItem('edufun_activity_logs',
      JSON.stringify(Array.isArray(data.activityLogs) ? data.activityLogs : []));
  ...
  // SELALU overwrite localStorage dari Firestore — tidak pernah
  // merge dengan data lama supaya ganti akun selalu bersih.
  // ============================================================
  function _applyStats(stats) {
    ...
    // ── Arena stat widget — SELALU overwrite dari Firestore ──
    const arenaData = {
    ...
    // ── Materi stat widget — SELALU overwrite dari Firestore ──
    const materiData = {
    ...
    // ── Pomo stat widget — SELALU overwrite dari Firestore ──
