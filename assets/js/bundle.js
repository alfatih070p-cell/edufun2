...
const FirebaseSync = (() => {
  ...
  async function _pullAllFromFirestore() {
    ...
    try {
      ...
      const doc = await fbDB.collection('users').doc(_uid).get();
      if (!doc.exists) {
        console.warn('[FirebaseSync] users doc tidak ada untuk UID:', _uid);
        _ready = true;
        setTimeout(() => {
      ...
      // ── Auto-migrate: isi field yang belum ada (akun lama dari 2 koleksi) ──
      const missing = {};
      if (data.coins  === undefined || data.coins  === null) missing.coins  = 0;
      if (data.accent === undefined || data.accent === null) missing.accent = '#3b82f6';
      if (!data.shop)                  missing.shop        = { owned: ['skin-1'], activeSkin: 'skin-1', activeAcc: '' };
      if (!Array.isArray(data.cards))  missing.cards       = [];
      if (!data.agenda || typeof data.agenda !== 'object') missing.agenda = {};
      if (!Array.isArray(data.xpHistory))    missing.xpHistory    = [];
      if (!Array.isArray(data.activityLogs)) missing.activityLogs = [];
      if (!data.streak || !Array.isArray(data.streak.days)) missing.streak = { days: [], lastShown: null };
      ...
      if (Object.keys(missing).length > 0) {
        ...
        fbDB.collection('users').doc(_uid).set(missing, { merge: true })
      ...
      // Render semua stat widget setelah data Firestore masuk
      setTimeout(() => {
        if (typeof renderAllStatWidgets === 'function') renderAllStatWidgets();
      }, 100);
    } catch (e) {
  ...
  function _startRealtimeListener() {
    if (!_uid) return;
    _unsubscribe = fbDB.collection('users').doc(_uid)
  ...
  function _applyAllToApp(data) {
    ...
    const shopData = data.shop || { owned: ['skin-1'], activeSkin: 'skin-1', activeAcc: '' };
    if (typeof window._shopOwned !== 'undefined') {
      window._shopOwned = Array.isArray(shopData.owned) ? shopData.owned : ['skin-1'];
      // Pastikan skin-1 selalu ada di owned
      if (!window._shopOwned.includes('skin-1')) window._shopOwned.unshift('skin-1');
    }
    if (typeof window._shopActiveSkin !== 'undefined') {
      window._shopActiveSkin = shopData.activeSkin || 'skin-1';
    }
    if (typeof window._shopActiveAcc !== 'undefined') {
      // Gunakan !== undefined agar string kosong '' tetap diapply
      window._shopActiveAcc = (shopData.activeAcc !== undefined) ? shopData.activeAcc : '';
    }
    if (typeof applyMascotSkin === 'function') applyMascotSkin();
    if (typeof renderShopButtons === 'function') renderShopButtons();
    // Render stat widgets setiap kali data Firestore diapply (termasuk realtime listener)
    if (typeof renderAllStatWidgets === 'function') setTimeout(renderAllStatWidgets, 50);
  }
  ...
  function _applyStatsToApp(stats) {
    ...
    else window._statArena = arenaData;
    ...
    else window._statPomo = pomoData;
  ...
  function flushAll() {
    ...
    // Streak dari StreakSystem memori — selalu include, ini data kritis
    try {
      if (typeof StreakSystem !== 'undefined' && StreakSystem.getDataInMemory) {
        const sd = StreakSystem.getDataInMemory();
        if (sd) {
          writes['streak'] = sd;
          // Pastikan streakCount juga ikut ter-flush ke stats
          if (typeof StreakSystem.calcStreak === 'function' && Array.isArray(sd.days)) {
            writes['stats.streakCount'] = StreakSystem.calcStreak(sd.days);
          }
        }
      }
    } catch(e) {}
    ...
    // Materi stat
    // CATATAN: stats.materiTotal TIDAK di-flush di sini karena sudah ditulis
    // via FieldValue.increment di onMateriOpen() setiap kali user buka materi.
    // Menulis ulang angka statis dari memori bisa menimpa nilai Firestore yang lebih tinggi.
    // Hanya flush metadata (mapel, bab, ts) sebagai safety net.
    try {
      if (window._statMateri) {
        const m = window._statMateri;
        if (m.lastMapel) writes['stats.lastMateriMapel'] = m.lastMapel;
        if (m.lastBab)   writes['stats.lastMateriBab']   = m.lastBab;
        if (m.lastTs)    writes['stats.lastMateriTs']    = m.lastTs;
      }
    } catch(e) {}
    ...
    // Tulis ke Firestore via batch (app.html tidak pernah unload sekarang karena pakai iframe)
    const batch = fbDB.batch();
    const ref   = fbDB.collection('users').doc(_uid);
  ...
  async function _setField(field, value) {
    if (!_uid) return;
    try {
      await fbDB.collection('users').doc(_uid).set({ [field]: value }, { merge: true });
  ...
  async function _mergeStats(partial) {
    if (!_uid) return;
    try {
      await fbDB.collection('users').doc(_uid).set({ stats: partial }, { merge: true });
  ...
  // saveStreak: LANGSUNG tulis ke Firestore tanpa debounce.
  // Streak adalah data kritis — jangan biarkan debounce timer tercancel saat user navigasi.
  async function saveStreak(streakData) {
    if (!_uid) return;
    clearTimeout(_timers['streak']); // batalkan debounce lama jika ada
    try {
      await fbDB.collection('users').doc(_uid).set({ streak: streakData }, { merge: true });
      console.log('[FirebaseSync] saveStreak OK — langsung tulis');
    } catch (e) {
      console.warn('[FirebaseSync] saveStreak error:', e);
    }
  }

  // saveStreakCount: LANGSUNG tulis stats.streakCount ke Firestore tanpa debounce.
  // Dipanggil segera setelah saveStreak agar tidak bisa tercancel oleh navigasi.
  async function saveStreakCount(count) {
    if (!_uid) return;
    clearTimeout(_timers['stats_streakCount']); // batalkan debounce lama jika ada
    try {
      await fbDB.collection('users').doc(_uid).set({ stats: { streakCount: count } }, { merge: true });
      console.log('[FirebaseSync] saveStreakCount OK —', count);
    } catch (e) {
      console.warn('[FirebaseSync] saveStreakCount error:', e);
    }
  }
  ...
  // Simpan agenda LANGSUNG ke Firestore tanpa debounce
  async function saveAgendaImmediate(agendaDB) {
    if (!_uid) return;
    clearTimeout(_timers['agenda']);
    try {
      await fbDB.collection('users').doc(_uid).set({ agenda: agendaDB }, { merge: true });
      console.log('[FirebaseSync] saveAgendaImmediate OK');
    } catch (e) {
      console.warn('[FirebaseSync] saveAgendaImmediate error:', e);
    }
  }
  ...
  function saveXPLevel(xpVal, levelVal) {
    _debounce('xp_level', async () => {
      if (!_uid) return;
      try {
        await fbDB.collection('users').doc(_uid).set({ stats: { xp: xpVal, level: levelVal } }, { merge: true });
        const session = (typeof Auth !== 'undefined') ? Auth.getSession() : null;
        const uname = session ? session.username : null;
        if (uname) {
          await fbDB.collection('leaderboard').doc(_uid).set(
            { uid: _uid, username: uname, xp: xpVal, level: levelVal, updatedAt: Date.now() },
            { merge: true }
          );
        }
  ...
  // Simpan data shop LANGSUNG ke Firestore tanpa debounce
  // Digunakan setelah beli/equip item agar tidak hilang saat user langsung keluar
  async function saveShopImmediate(shopData) {
    if (!_uid) return;
    // Cancel debounce yang mungkin masih pending
    clearTimeout(_timers['shop']);
    try {
      await fbDB.collection('users').doc(_uid).set({ shop: shopData }, { merge: true });
      console.log('[FirebaseSync] saveShopImmediate OK:', shopData);
    } catch (e) {
      console.warn('[FirebaseSync] saveShopImmediate error:', e);
    }
  }
  ...
  function onPomodoroComplete() {
    ...
    _debounce('pomo_complete', async () => {
      if (!_uid) return;
      try {
        const snap = await fbDB.collection('users').doc(_uid).get();
        ...
        await fbDB.collection('users').doc(_uid).set({
  ...
  function onMateriOpen(mapel, bab) {
    ...
    const inc = firebase.firestore.FieldValue.increment;
    const ts  = Date.now();
    // Update in-memory _statMateri SEBELUM Firestore write,
    // sehingga renderMateriStatWidget() langsung menampilkan nilai terbaru
    // dan flushAll() tidak menimpa dengan nilai lama.
    if (typeof window._statMateri !== 'undefined') {
      const prev = window._statMateri || { total: 0, lastMapel: null, lastBab: null, lastTs: null };
      window._statMateri = {
        total:     (prev.total || 0) + 1,
        lastMapel: mapel || prev.lastMapel,
        lastBab:   bab   || prev.lastBab,
        lastTs:    ts,
        lastOnlineTs: prev.lastOnlineTs || null,
      };
    }
    fbDB.collection('users').doc(_uid).set({
      stats: { materiTotal: inc(1), lastMateriMapel: mapel||null, lastMateriBab: bab||null, lastMateriTs: ts }
  ...
  return {
    ...
    saveCards, saveAgenda, saveAgendaImmediate, saveStreak, saveStreakCount, saveAccent,
    saveCharacter, saveStats, saveStatsField, saveXPLevel,
    saveXPHistory, saveActivityLog, saveMapelProgress,
    saveCoins, saveShop, saveShopImmediate,
...
const GeminiQuiz = (() => {
  ...
  const fallbackQs = {
    ...
    Kimia: [
      ...
      { q: 'Simbol unsur besi (Iron) adalah?', o: ['Fe','Ir','Be','Bi'], a: 0 },
      { q: 'Massa atom relatif karbon-12 adalah?', o: ['6','10','12','14'], a: 2 },
      { q: 'Reaksi kimia yang membutuhkan energi panas disebut?', o: ['Eksoterm','Endoterm','Redoks','Ionisasi'], a: 1 },
      { q: 'Bilangan oksidasi O dalam H₂O adalah?', o: ['+1','-1','-2','+2'], a: 2 },
      { q: 'Larutan basa memiliki pH?', o: ['< 7','= 7','> 7','= 0'], a: 2 },
      { q: 'Gas yang dihasilkan saat logam besi bereaksi dengan asam klorida adalah?', o: ['O₂','CO₂','H₂','N₂'], a: 2 },
      { q: 'Jumlah atom dalam 1 mol zat (bilangan Avogadro) adalah?', o: ['6,02×10²¹','6,02×10²³','6,02×10²⁵','6,02×10²⁰'], a: 1 },
      { q: 'Unsur halogen dengan nomor atom 17 adalah?', o: ['Fluor','Brom','Klorin','Iodin'], a: 2 },
      { q: 'Persamaan reaksi: H₂ + O₂ → H₂O. Koefisien yang benar adalah?', o: ['1,1,1','2,1,2','1,2,2','2,2,1'], a: 1 },
      { q: 'pH air murni pada 25°C adalah?', o: ['5','6','7','8'], a: 2 },
      { q: 'Elektron valensi atom Natrium (Na, nomor atom 11) adalah?', o: ['1','2','3','8'], a: 0 },
      { q: 'Reaksi pembakaran sempurna menghasilkan?', o: ['CO & H₂O','CO₂ & H₂O','CO₂ & H₂','C & H₂O'], a: 1 },
      { q: 'Senyawa organik paling sederhana adalah?', o: ['Etana','Propana','Metana','Butana'], a: 2 },
    ...
    'Biologi': [
      ...
      { q: 'Proses tumbuhan membuat makanan sendiri menggunakan sinar matahari disebut?', o: ['Respirasi','Fotosintesis','Fermentasi','Transpirasi'], a: 1 },
      { q: 'Bagian sel tumbuhan yang tidak ada pada sel hewan adalah?', o: ['Mitokondria','Ribosom','Dinding sel','Membran sel'], a: 2 },
      { q: 'Sistem yang bertanggung jawab melawan penyakit dalam tubuh adalah?', o: ['Sistem saraf','Sistem imun','Sistem endokrin','Sistem ekskresi'], a: 1 },
      { q: 'Urutan taksonomi dari yang paling luas ke paling sempit adalah?', o: ['Kingdom-Filum-Kelas-Ordo-Famili-Genus-Spesies','Spesies-Genus-Famili-Ordo-Kelas-Filum-Kingdom','Kingdom-Kelas-Filum-Ordo-Famili-Genus-Spesies','Filum-Kingdom-Kelas-Ordo-Genus-Famili-Spesies'], a: 0 },
      { q: 'Respirasi seluler terjadi di organel?', o: ['Kloroplas','Nukleus','Mitokondria','Vakuola'], a: 2 },
      { q: 'Nama ilmiah manusia adalah?', o: ['Homo habilis','Homo sapiens','Homo erectus','Homo neanderthalensis'], a: 1 },
      { q: 'Bagian darah yang berfungsi membawa oksigen adalah?', o: ['Leukosit','Trombosit','Eritrosit','Plasma'], a: 2 },
      { q: 'Proses pengeluaran zat sisa metabolisme dari tubuh disebut?', o: ['Sekresi','Ekskresi','Defekasi','Respirasi'], a: 1 },
      { q: 'Vitamin yang larut dalam lemak adalah?', o: ['Vitamin C','Vitamin B','Vitamin A','Vitamin B12'], a: 2 },
      { q: 'Bagian otak yang mengatur keseimbangan tubuh adalah?', o: ['Serebrum','Serebelum','Medula oblongata','Pons'], a: 1 },
      { q: 'Proses mutasi gen dapat terjadi akibat?', o: ['Olahraga berlebihan','Paparan radiasi','Kurang tidur','Makan banyak'], a: 1 },
      { q: 'Golongan darah yang disebut "donor universal" adalah?', o: ['A','B','AB','O'], a: 3 },
      { q: 'Organel yang berperan dalam sintesis protein adalah?', o: ['Lisosom','Mitokondria','Ribosom','Vakuola'], a: 2 },
      { q: 'Rantai makanan yang benar adalah?', o: ['Harimau→Rusa→Rumput','Rumput→Belalang→Katak→Ular','Katak→Ular→Rumput→Belalang','Ular→Katak→Belalang→Harimau'], a: 1 },
    ],
  };
  ...
  async function getQuestions(subject, difficulty = 'Sedang') {
    console.info('[Quiz] Mode offline — memuat soal lokal, mapel:', subject);

    const subjectPool = fallbackQs[subject] || fallbackQs['Matematika'];

    // Kumpulkan soal dari semua mapel lain untuk bumbu campuran
    const otherPools = Object.entries(fallbackQs)
      .filter(([key]) => key !== subject)
      .flatMap(([, qs]) => qs);

    // Ambil 60% dari mapel utama, 40% campuran (minimal 10 soal total)
    const mainCount  = Math.max(6, Math.ceil(subjectPool.length * 0.6));
    const mixCount   = Math.max(4, Math.floor(mainCount * 0.67));

    const mainPicked = _shuffle([...subjectPool]).slice(0, mainCount);
    const mixPicked  = _shuffle([...otherPools]).slice(0, mixCount);

    return _shuffle([...mainPicked, ...mixPicked]);
  }
...
// STATE
// ===========================
let ch = { gender: 'male', skin: '#FDDBB4', hair: 'short', hairColor: '#3b2507', outfit: 'school', outfitColor: null, acc: 'none', acc2: 'none', bottomStyle: 'pants', bottomColor: '#1e3a5f' };
let userName = 'Pelajar', userRole = 'siswa';
...
function applyAc(hex) {
  ...
  try { localStorage.setItem('ef_ac', hex); } catch(e) {}
...
// Terapkan accent TANPA menulis kembali ke Firestore (dipakai oleh firebase-sync untuk hindari loop)
function _applyAccentOnly(hex) {
  const r = hexRgb(hex);
  const d = document.documentElement;
  d.style.setProperty('--ac', hex);
  d.style.setProperty('--ac-rgb', r);
  ...
  const ch2 = document.getElementById('color-hex');
  if (cp) cp.value = hex;
...
// Terapkan accent langsung dari session (sudah di-pull dari Firestore saat login).
// FirebaseSync akan konfirmasi ulang setelah pull selesai.
(function() {
  // Accent default dulu — Firestore akan apply warna yang benar via _applyAllToApp
  _applyAccentOnly('#3b82f6');
})();
...
function renderAv(svgId) {
  ...
  const skin = ch.skin;
  const hc = ch.hairColor || '#3b2507';
  const hairLong = ch.hair === 'long';
  const hairCurly = ch.hair === 'curly';
  // Outfit color: custom override or default by type
  let oc, oc2;
  if (ch.outfitColor) {
    oc = ch.outfitColor;
    oc2 = '#f1f5f9';
  } else {
    oc = ch.outfit === 'school' ? (isMale ? '#2563eb' : '#dc2626') : (ch.outfit === 'hoodie' ? '#6d28d9' : '#64748b');
    oc2 = ch.outfit === 'school' ? '#f1f5f9' : '#d1d5db';
  }
  let hair = '';
  if (hairCurly) {
  ...
  if (ch.acc === 'sunglasses') acc = `<rect x="40" y="50" width="16" height="11" rx="5" fill="#0f172a" opacity=".85"/><rect x="61" y="50" width="16" height="11" rx="5" fill="#0f172a" opacity=".85"/><line x1="56" y1="55" x2="61" y2="55" stroke="#475569" stroke-width="1.5"/><line x1="40" y1="55" x2="35" y2="54" stroke="#475569" stroke-width="1.5"/><line x1="77" y1="55" x2="82" y2="54" stroke="#475569" stroke-width="1.5"/>`;
  if (ch.acc === 'headband') acc = `<rect x="36" y="26" width="48" height="10" rx="5" fill="#e11d48"/><circle cx="60" cy="23" r="6" fill="#fb7185"/>`;
  if (ch.acc === 'crown') acc = `<polygon points="36,28 44,14 52,24 60,10 68,24 76,14 84,28" fill="#f59e0b" stroke="#d97706" stroke-width="1.5"/><rect x="36" y="26" width="48" height="8" rx="3" fill="#fbbf24"/>`;
  // Accessories layer 2 (body) - smooth SVG shapes only
  let acc2 = '';
  if (ch.acc2 === 'scarf') acc2 = `<rect x="40" y="78" width="40" height="14" rx="6" fill="#ef4444"/><path d="M55,92 Q58,100 54,108" stroke="#ef4444" stroke-width="8" stroke-linecap="round" fill="none"/>`;
  if (ch.acc2 === 'necktie') acc2 = `<polygon points="60,80 56,88 60,120 64,88" fill="#1e1e1e"/><polygon points="56,80 64,80 62,86 58,86" fill="#111111"/>`;
  if (ch.acc2 === 'bow') acc2 = `<ellipse cx="52" cy="83" rx="8" ry="5" fill="#1e1e1e"/><ellipse cx="68" cy="83" rx="8" ry="5" fill="#1e1e1e"/><circle cx="60" cy="83" r="4" fill="#444444"/>`;
  if (ch.acc2 === 'bag') acc2 = `<rect x="74" y="86" width="16" height="22" rx="5" fill="#7c3aed"/><rect x="76" y="90" width="12" height="7" rx="3" fill="#8b5cf6"/><rect x="74" y="89" width="3" height="18" rx="1.5" fill="#6d28d9"/><rect x="87" y="89" width="3" height="18" rx="1.5" fill="#6d28d9"/>`;
  if (ch.acc2 === 'wings') acc2 = `<path d="M28,90 Q10,75 15,110 Q28,100 36,95 Z" fill="#a5f3fc" opacity=".9"/><path d="M92,90 Q110,75 105,110 Q92,100 84,95 Z" fill="#a5f3fc" opacity=".9"/>`;
  if (ch.acc2 === 'cape') acc2 = `<path d="M36,82 Q28,120 40,145 L60,135 L80,145 Q92,120 84,82 Z" fill="#7c3aed" opacity=".85"/>`;
  // Bottom (pants/skirt/shorts/longskirt)
  const bc = ch.bottomColor || (isMale ? '#1e3a5f' : '#881337');
  const bs = ch.bottomStyle || 'pants';
  let bottom = '';
  let legs = ''; // kaki terpisah agar bisa ditambah untuk rok
  if (bs === 'pants') {
    bottom = `<rect x="38" y="138" width="20" height="38" rx="6" fill="${bc}"/><rect x="62" y="138" width="20" height="38" rx="6" fill="${bc}"/>`;
    legs = `<rect x="38" y="168" width="20" height="8" rx="4" fill="#334155"/><rect x="62" y="168" width="20" height="8" rx="4" fill="#334155"/>`;
  } else if (bs === 'shorts') {
    bottom = `<rect x="38" y="138" width="20" height="22" rx="6" fill="${bc}"/><rect x="62" y="138" width="20" height="22" rx="6" fill="${bc}"/>`;
    // Shorts: tampilkan kaki kulit dari bawah shorts
    legs = `<rect x="40" y="160" width="16" height="28" rx="6" fill="${skin}"/><rect x="64" y="160" width="16" height="28" rx="6" fill="${skin}"/><rect x="38" y="182" width="20" height="8" rx="4" fill="#334155"/><rect x="62" y="182" width="20" height="8" rx="4" fill="#334155"/>`;
  } else if (bs === 'skirt') {
    bottom = `<path d="M36,138 Q40,176 60,176 Q80,176 84,138 Z" fill="${bc}"/>`;
    // Rok mini: tampilkan kaki kulit yang muncul di bawah rok
    legs = `<rect x="40" y="168" width="16" height="22" rx="6" fill="${skin}"/><rect x="64" y="168" width="16" height="22" rx="6" fill="${skin}"/><rect x="38" y="184" width="20" height="8" rx="4" fill="#334155"/><rect x="62" y="184" width="20" height="8" rx="4" fill="#334155"/>`;
  } else if (bs === 'longskirt') {
    bottom = `<path d="M36,138 Q32,182 60,182 Q88,182 84,138 Z" fill="${bc}"/>`;
    // Rok panjang: tampilkan ujung kaki (kaki kiri kanan) di bawah rok panjang
    legs = `<rect x="41" y="178" width="15" height="12" rx="5" fill="${skin}"/><rect x="64" y="178" width="15" height="12" rx="5" fill="${skin}"/><rect x="39" y="186" width="19" height="8" rx="4" fill="#334155"/><rect x="62" y="186" width="19" height="8" rx="4" fill="#334155"/>`;
  }
  // Outfit body: setiap tipe punya bentuk SVG yang berbeda signifikan
  let outfitBody = '';
  if (ch.outfit === 'school') {
    // Seragam: kemeja formal berkerah + garis seragam
    outfitBody = `
      <rect x="36" y="80" width="48" height="62" rx="8" fill="${oc}"/>
      <rect x="28" y="82" width="14" height="48" rx="7" fill="${oc}"/>
      <rect x="78" y="82" width="14" height="48" rx="7" fill="${oc}"/>
      <polygon points="48,80 60,92 72,80" fill="${oc2}"/>
      <rect x="55" y="80" width="10" height="30" rx="3" fill="${oc2}" opacity=".7"/>
      <rect x="57" y="82" width="6" height="4" rx="1.5" fill="${oc}" opacity=".6"/>
      <rect x="57" y="90" width="6" height="4" rx="1.5" fill="${oc}" opacity=".6"/>
      <rect x="57" y="98" width="6" height="4" rx="1.5" fill="${oc}" opacity=".6"/>
      <rect x="36" y="135" width="48" height="7" rx="3" fill="${oc2}" opacity=".5"/>
    `;
  } else if (ch.outfit === 'casual') {
    // Kasual: kaos polos dengan garis horizontal dan leher bulat
    const lighterOc = oc + 'cc';
    outfitBody = `
      <rect x="36" y="82" width="48" height="62" rx="12" fill="${oc}"/>
      <rect x="28" y="84" width="15" height="46" rx="8" fill="${oc}"/>
      <rect x="77" y="84" width="15" height="46" rx="8" fill="${oc}"/>
      <ellipse cx="60" cy="82" rx="12" ry="6" fill="${oc2}"/>
      <rect x="40" y="100" width="40" height="5" rx="2.5" fill="${oc2}" opacity=".35"/>
      <rect x="40" y="112" width="40" height="5" rx="2.5" fill="${oc2}" opacity=".35"/>
      <rect x="40" y="124" width="40" height="5" rx="2.5" fill="${oc2}" opacity=".2"/>
      <rect x="44" y="84" width="32" height="14" rx="7" fill="${oc2}" opacity=".15"/>
    `;
  } else if (ch.outfit === 'hoodie') {
    // Hoodie: jaket bertudung dengan kantong depan besar & zipper
    const hoodieShade = oc + 'dd';
    outfitBody = `
      <rect x="34" y="80" width="52" height="65" rx="14" fill="${oc}"/>
      <rect x="26" y="82" width="16" height="52" rx="8" fill="${oc}"/>
      <rect x="78" y="82" width="16" height="52" rx="8" fill="${oc}"/>
      <path d="M42,80 Q60,72 78,80 Q74,95 60,98 Q46,95 42,80 Z" fill="${oc}" stroke="${oc2}" stroke-width="2"/>
      <line x1="60" y1="80" x2="60" y2="145" stroke="${oc2}" stroke-width="2.5" stroke-dasharray="4,3"/>
      <rect x="43" y="118" width="34" height="22" rx="8" fill="${oc2}" opacity=".25"/>
      <rect x="48" y="122" width="24" height="14" rx="5" fill="${oc2}" opacity=".2"/>
      <circle cx="60" cy="129" r="3" fill="${oc2}" opacity=".5"/>
    `;
  }
  svg.innerHTML = `
    ${hair}
...
// MODALS / DROPDOWNS
// ===========================
function openModal(id) { SFX.modalOpen(); document.getElementById(id).classList.add('on'); if (id === 'settings-modal') { setTimeout(renderSettingsStats, 80); } }
...
// ===========================
function addXP(amt) {
  // Log XP change for history chart — simpan ke memori global + Firestore
  try {
    if (!Array.isArray(window._xpHistory)) window._xpHistory = [];
    window._xpHistory.push({ ts: Date.now(), amt: amt, xpAfter: Math.max(0, xp + amt) });
    if (window._xpHistory.length > 500) window._xpHistory.splice(0, window._xpHistory.length - 500);
    if (typeof FirebaseSync !== 'undefined' && FirebaseSync.saveXPHistory) {
      FirebaseSync.saveXPHistory(window._xpHistory);
    }
  } catch(e) {}
...
function updateCoinUI() {
  const c = document.getElementById('coin-lbl');
  if (c) c.textContent = coins;
  const cm = document.getElementById('mob-coin-lbl');
  if (cm) cm.textContent = coins;
}

function addCoins(n) {
  coins += n;
  if (typeof FirebaseSync !== 'undefined') FirebaseSync.saveCoins(coins);
  updateCoinUI();
  toast('+' + n + ' Koin! Total: ' + coins + ' koin');
}
...
function logActivity(mapel, mnt, type) {
  ...
  try {
    if (!Array.isArray(window._activityLogs)) window._activityLogs = [];
    window._activityLogs.push({ ts: Date.now(), mapel: normalizedMapel, mnt: mnt || 1, type: type || 'study' });
    if (window._activityLogs.length > 1000) window._activityLogs.splice(0, window._activityLogs.length - 1000);
    if (typeof FirebaseSync !== 'undefined' && FirebaseSync.saveActivityLog) {
      FirebaseSync.saveActivityLog(window._activityLogs);
    }
  } catch(e) {}
}
...
// FLASHCARDS
// ===========================
let cards = []; // diisi oleh FirebaseSync setelah pull dari Firestore
...
function nextFC() {
  ...
  if (wasLast && cards.length > 0) {
    // Selesai satu putaran flashcard — catat sebagai sesi belajar
    const fcMapelName = currentFcMapel !== 'semua' ? currentFcMapel : 'Flashcard';
    if (typeof materiStatRecord === 'function') materiStatRecord(fcMapelName, 'Flashcard');
    if (typeof FirebaseSync !== 'undefined') {
      FirebaseSync.onFlashStudyDone(cards.length, cards.length, currentFcMapel !== 'semua' ? currentFcMapel : null);
      FirebaseSync.saveXPLevel(xp, level);
    }
  }
}
...
window._refreshFlashCards = function(freshCards) {
  const fresh = Array.isArray(freshCards) ? freshCards : [];
...
function renderAvWithData(svgId, data) {
  ...
  if (data.acc === 'sunglasses') acc = `<rect x="40" y="50" width="16" height="11" rx="5" fill="#0f172a" opacity=".85"/><rect x="61" y="50" width="16" height="11" rx="5" fill="#0f172a" opacity=".85"/><line x1="56" y1="55" x2="61" y2="55" stroke="#475569" stroke-width="1.5"/>`;
  if (data.acc === 'headband') acc = `<rect x="36" y="26" width="48" height="10" rx="5" fill="#e11d48"/><circle cx="60" cy="23" r="6" fill="#fb7185"/>`;
  if (data.acc === 'crown') acc = `<polygon points="36,28 44,14 52,24 60,10 68,24 76,14 84,28" fill="#f59e0b" stroke="#d97706" stroke-width="1.5"/><rect x="36" y="26" width="48" height="8" rx="3" fill="#fbbf24"/>`;
  let dacc2 = '';
  if (data.acc2 === 'scarf') dacc2 = `<rect x="40" y="78" width="40" height="14" rx="6" fill="#ef4444"/><path d="M55,92 Q58,100 54,108" stroke="#ef4444" stroke-width="8" stroke-linecap="round" fill="none"/>`;
  if (data.acc2 === 'necktie') dacc2 = `<polygon points="60,80 56,88 60,120 64,88" fill="#1e1e1e"/><polygon points="56,80 64,80 62,86 58,86" fill="#111111"/>`;
  if (data.acc2 === 'bow') dacc2 = `<ellipse cx="52" cy="83" rx="8" ry="5" fill="#1e1e1e"/><ellipse cx="68" cy="83" rx="8" ry="5" fill="#1e1e1e"/><circle cx="60" cy="83" r="4" fill="#444444"/>`;
  if (data.acc2 === 'bag') dacc2 = `<rect x="74" y="86" width="16" height="22" rx="5" fill="#7c3aed"/><rect x="76" y="90" width="12" height="7" rx="3" fill="#8b5cf6"/>`;
  if (data.acc2 === 'wings') dacc2 = `<path d="M28,90 Q10,75 15,110 Q28,100 36,95 Z" fill="#a5f3fc" opacity=".9"/><path d="M92,90 Q110,75 105,110 Q92,100 84,95 Z" fill="#a5f3fc" opacity=".9"/>`;
  if (data.acc2 === 'cape') dacc2 = `<path d="M36,82 Q28,120 40,145 L60,135 L80,145 Q92,120 84,82 Z" fill="#7c3aed" opacity=".85"/>`;
  const dbc = data.bottomColor || (isMale ? '#1e3a5f' : '#881337');
  const dbs = data.bottomStyle || 'pants';
  let dbottom = '';
  let dlegs = '';
  if (dbs === 'pants') {
    dbottom = `<rect x="38" y="138" width="20" height="38" rx="6" fill="${dbc}"/><rect x="62" y="138" width="20" height="38" rx="6" fill="${dbc}"/>`;
    dlegs = `<rect x="38" y="168" width="20" height="8" rx="4" fill="#334155"/><rect x="62" y="168" width="20" height="8" rx="4" fill="#334155"/>`;
  } else if (dbs === 'shorts') {
    dbottom = `<rect x="38" y="138" width="20" height="22" rx="6" fill="${dbc}"/><rect x="62" y="138" width="20" height="22" rx="6" fill="${dbc}"/>`;
    dlegs = `<rect x="40" y="160" width="16" height="28" rx="6" fill="${skin}"/><rect x="64" y="160" width="16" height="28" rx="6" fill="${skin}"/><rect x="38" y="182" width="20" height="8" rx="4" fill="#334155"/><rect x="62" y="182" width="20" height="8" rx="4" fill="#334155"/>`;
  } else if (dbs === 'skirt') {
    dbottom = `<path d="M36,138 Q40,176 60,176 Q80,176 84,138 Z" fill="${dbc}"/>`;
    dlegs = `<rect x="40" y="168" width="16" height="22" rx="6" fill="${skin}"/><rect x="64" y="168" width="16" height="22" rx="6" fill="${skin}"/><rect x="38" y="184" width="20" height="8" rx="4" fill="#334155"/><rect x="62" y="184" width="20" height="8" rx="4" fill="#334155"/>`;
  } else if (dbs === 'longskirt') {
    dbottom = `<path d="M36,138 Q32,182 60,182 Q88,182 84,138 Z" fill="${dbc}"/>`;
    dlegs = `<rect x="41" y="178" width="15" height="12" rx="5" fill="${skin}"/><rect x="64" y="178" width="15" height="12" rx="5" fill="${skin}"/><rect x="39" y="186" width="19" height="8" rx="4" fill="#334155"/><rect x="62" y="186" width="19" height="8" rx="4" fill="#334155"/>`;
  }
  let doutfitBody = '';
  const doc2 = data.outfit === 'school' ? '#f1f5f9' : '#d1d5db';
  if (data.outfit === 'school') {
    doutfitBody = `
      <rect x="36" y="80" width="48" height="62" rx="8" fill="${oc}"/>
      <rect x="28" y="82" width="14" height="48" rx="7" fill="${oc}"/>
      <rect x="78" y="82" width="14" height="48" rx="7" fill="${oc}"/>
      <polygon points="48,80 60,92 72,80" fill="${doc2}"/>
      <rect x="55" y="80" width="10" height="30" rx="3" fill="${doc2}" opacity=".7"/>
      <rect x="57" y="82" width="6" height="4" rx="1.5" fill="${oc}" opacity=".6"/>
      <rect x="57" y="90" width="6" height="4" rx="1.5" fill="${oc}" opacity=".6"/>
      <rect x="57" y="98" width="6" height="4" rx="1.5" fill="${oc}" opacity=".6"/>
      <rect x="36" y="135" width="48" height="7" rx="3" fill="${doc2}" opacity=".5"/>
    `;
  } else if (data.outfit === 'casual') {
    doutfitBody = `
      <rect x="36" y="82" width="48" height="62" rx="12" fill="${oc}"/>
      <rect x="28" y="84" width="15" height="46" rx="8" fill="${oc}"/>
      <rect x="77" y="84" width="15" height="46" rx="8" fill="${oc}"/>
      <ellipse cx="60" cy="82" rx="12" ry="6" fill="${doc2}"/>
      <rect x="40" y="100" width="40" height="5" rx="2.5" fill="${doc2}" opacity=".35"/>
      <rect x="40" y="112" width="40" height="5" rx="2.5" fill="${doc2}" opacity=".35"/>
      <rect x="40" y="124" width="40" height="5" rx="2.5" fill="${doc2}" opacity=".2"/>
    `;
  } else {
    doutfitBody = `
      <rect x="34" y="80" width="52" height="65" rx="14" fill="${oc}"/>
      <rect x="26" y="82" width="16" height="52" rx="8" fill="${oc}"/>
      <rect x="78" y="82" width="16" height="52" rx="8" fill="${oc}"/>
      <path d="M42,80 Q60,72 78,80 Q74,95 60,98 Q46,95 42,80 Z" fill="${oc}" stroke="${doc2}" stroke-width="2"/>
      <line x1="60" y1="80" x2="60" y2="145" stroke="${doc2}" stroke-width="2.5" stroke-dasharray="4,3"/>
      <rect x="43" y="118" width="34" height="22" rx="8" fill="${doc2}" opacity=".25"/>
      <circle cx="60" cy="129" r="3" fill="${doc2}" opacity=".5"/>
    `;
  }
  svg.innerHTML = `
    ${hair}
...
function generateRandomChar() {
  ...
  const acc2s = ['none','none','scarf','necktie','bow'];
  const hairColors = ['#3b2507','#1a1a1a','#7c3d12','#b45309','#dc2626','#7c3aed'];
  const genders = ['male','female'];
  const bottoms = ['pants','shorts','skirt'];
  const bottomColors = ['#1e3a5f','#881337','#374151','#0f766e','#7c3aed'];
  return {
    ...
    acc2: acc2s[Math.floor(Math.random() * acc2s.length)],
...
// ═══════════════════════════════════════════════════════
let brMapel = 'Matematika', brLevel = 'Mudah', brCode = '', brPollIv = null;
...
async function generatePrivateRoom() {
  ...
  try {
    ...
    document.getElementById('br-code-meta').textContent = brMapel + ' · ' + brLevel + ' · Auto-hapus jika kosong';
...
// Setiap soal punya ID unik. Bot setTimeout menyimpan ID ini saat dibuat.
// Ketika bot setTimeout menembak, dicek dulu apakah ID masih cocok (soal belum berganti).
// Ini mencegah bot dari soal lama men-set btAnswered=true untuk soal baru.
let btQGen = 0;

function renderBQ() {
  if (btQ >= window._btQs.length) btQ = 0;
  btAnswered = false;
  btQGen++;                        // naikkan ID soal saat ini
  const myGen = btQGen;            // tutup di closure bot
  const q = window._btQs[btQ];
  // Acak posisi jawaban setiap render (shuffleOptions tidak mengubah data asli)
  const shuffled = _shuffleOptions(q);
  document.getElementById('bt-q').textContent = shuffled.q;
  const opts = document.getElementById('bt-opts');
  opts.innerHTML = shuffled.o.map((o, i) => `<button onclick="answerBattle(${i})" style="padding:.5rem;border:1.5px solid #e2e8f0;border-radius:10px;background:#fff;font-weight:700;font-size:.82rem;cursor:pointer;font-family:inherit;transition:all .15s;text-align:left" onmouseover="this.style.borderColor='var(--ac)'" onmouseout="if(!this.dataset.locked)this.style.borderColor='#e2e8f0'">${o}</button>`).join('');
  // Simpan indeks jawaban benar hasil shuffle ke soal sementara
  window._btCurrentQ = shuffled;
  ...
  setTimeout(() => {
    // Batalkan jika soal sudah berganti (ID tidak cocok) atau sudah dijawab player
    if (myGen !== btQGen || btAnswered) return;
...
// Acak posisi pilihan jawaban tanpa mengubah data soal asli
function _shuffleOptions(q) {
  const indices = [0, 1, 2, 3].slice(0, q.o.length);
  // Fisher-Yates pada array index
  for (let i = indices.length - 1; i > 0; i--) {
    const j = Math.floor(Math.random() * (i + 1));
    [indices[i], indices[j]] = [indices[j], indices[i]];
  }
  const newOpts  = indices.map(i => q.o[i]);
  const newAns   = indices.indexOf(q.a);
  return { q: q.q, o: newOpts, a: newAns };
}

function answerBattle(idx) {
  if (btAnswered) return;
  btAnswered = true; clearInterval(btInterval);
  // Pakai soal yang sudah di-shuffle (posisi jawaban bisa berbeda dari data asli)
  const q = window._btCurrentQ || window._btQs[btQ];
...
function finishBattle() {
  ...
  if (won) { SFX.win(); addXP(btXpWin); addCoins(2); const el = document.getElementById('lb-my-xp'); el.textContent = parseInt(el.textContent) + btXpWin; }
  ...
  // NOTE: onBattleResult dipanggil dari hook arenaStatRecord (bawah) — JANGAN panggil lagi di sini
  if (typeof FirebaseSync !== 'undefined') {
    FirebaseSync.saveXPLevel(xp, level);
  }
...
function renderMateri() {
  ...
  // NEW! badge config: which subjects are newly accessible (only Kelas 11 Fisika & Matematika)
  const newSubjects = { '11': ['Fisika', 'Matematika'] };
  const accessibleBabs = {
    '11-Fisika': ['Fluida Statis (Tekanan & Archimedes)','Fluida Dinamis (Bernoulli)','Suhu & Kalor','Perpindahan Kalor','Teori Kinetik Gas','Termodinamika','Gelombang Mekanik','Gelombang Bunyi','Optik Geometri (Cermin & Lensa)'],
    '11-Matematika': ['Fungsi & Fungsi Invers','Komposisi Fungsi','Matriks & Operasinya','Determinan & Invers Matriks','Vektor di Bidang & Ruang','Transformasi Geometri','Barisan & Deret Aritmetika','Barisan & Deret Geometri','Limit Fungsi Aljabar']
  };

  // Determine which subjects are fully unlocked (accessible)
  const unlockedSubjects = { '11': ['Fisika', 'Matematika'] };
  ...
  document.getElementById('materi-content').innerHTML = Object.entries(data).map(([subj, babs]) => {
    ...
    const isSubjUnlocked = (unlockedSubjects[curKelas] || []).includes(subj);
    // Subject is locked if it's not in unlockedSubjects
    const isSubjLocked = !isSubjUnlocked;
    ...
    const lockBadge = isSubjLocked ? `<span style="background:#f1f5f9;color:#94a3b8;font-size:.58rem;font-weight:700;padding:.1rem .32rem;border-radius:99px;margin-left:.3rem;vertical-align:middle;border:1px solid #e2e8f0">🔒 Segera</span>` : '';

    const subjClickAction = isSubjLocked ? `selectSubj('${subj}')` : `toggleAcc('${key}')`;
    const subjBgStyle = isSubjLocked ? 'background:#fafbfc;' : 'background:#fff;';
    const subjBorderStyle = isSubjLocked ? 'border:1.5px solid #e2e8f0;opacity:.85;' : 'border:1.5px solid #e2e8f0;';

    return `<div style="${subjBgStyle}${subjBorderStyle}border-radius:16px;overflow:hidden">
      <div onclick="${subjClickAction}" style="display:flex;align-items:center;gap:.7rem;padding:.9rem 1rem;cursor:pointer;transition:background .15s" onmouseover="this.style.background='#f8fafc'" onmouseout="this.style.background='${isSubjLocked ? '#fafbfc' : 'transparent'}'">
        <div style="font-size:1.3rem;${isSubjLocked ? 'filter:grayscale(.5);' : ''}">${icon}</div>
        <div style="flex:1"><div style="font-weight:800;font-size:.88rem;${isSubjLocked ? 'color:#94a3b8;' : ''}">${subj}${newBadge}${lockBadge}</div><div style="font-size:.72rem;color:#94a3b8">${babs.length} bab · Kelas ${curKelas}</div></div>
        <div style="display:flex;align-items:center;gap:.4rem">
          ${isSubjLocked
            ? `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#cbd5e1" stroke-width="2.5"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>`
            : `<div class="badge ba">${babs.length}</div><svg id="arr-${key}" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#94a3b8" stroke-width="2.5" style="transition:transform .2s;flex-shrink:0"><polyline points="9 18 15 12 9 6"/></svg>`
          }
        </div>
      </div>
      ${!isSubjLocked ? `<div class="acc-b" id="acc-${key}">
...
function selectSubj(subj) {
  ...
  modal.innerHTML = `
    <div style="background:#fff;border-radius:24px;padding:2rem 1.75rem 1.75rem;max-width:300px;width:100%;text-align:center;box-shadow:0 20px 60px rgba(0,0,0,.2);position:relative">
      <div style="font-size:3.2rem;margin-bottom:.75rem">🔒</div>
      <div style="font-size:1.2rem;font-weight:900;color:#1e1b4b;margin-bottom:.4rem">Segera Hadir!</div>
      <div style="font-size:.88rem;font-weight:700;color:#334155;margin-bottom:.3rem">${subj}</div>
      <div style="font-size:.78rem;color:#64748b;margin-bottom:1.5rem;line-height:1.5">sedang dalam tahap pengembangan dan<br>akan segera tersedia.<br><br>Nantikan update berikutnya! 🚀</div>
      <button onclick="document.getElementById('coming-soon-modal').remove()" style="background:#3b82f6;color:#fff;border:none;border-radius:12px;padding:.7rem 0;font-weight:800;font-size:.92rem;cursor:pointer;width:100%">Oke, Mengerti!</button>
    </div>
  `;
...
function openBab(subj, bab) {
  const babPages = {
    // Kelas 11 Fisika
    'Termodinamika': '11/fisika/termodinamika.html',
    'Fluida Statis (Tekanan & Archimedes)': '11/fisika/fluida-statis.html',
    'Fluida Dinamis (Bernoulli)': '11/fisika/fluida-dinamis.html',
    'Suhu & Kalor': '11/fisika/suhu-kalor.html',
    'Perpindahan Kalor': '11/fisika/perpindahan-kalor.html',
    'Teori Kinetik Gas': '11/fisika/teori-kinetik-gas.html',
    'Gelombang Mekanik': '11/fisika/gelombang-mekanik.html',
    'Gelombang Bunyi': '11/fisika/gelombang-bunyi.html',
    'Optik Geometri (Cermin & Lensa)': '11/fisika/optik-geometri.html',
    // Kelas 11 Matematika
    'Fungsi & Fungsi Invers': '11/matematika/fungsi-invers.html',
    'Komposisi Fungsi': '11/matematika/komposisi-fungsi.html',
    'Matriks & Operasinya': '11/matematika/matriks-operasi.html',
    'Determinan & Invers Matriks': '11/matematika/determinan-invers.html',
    'Vektor di Bidang & Ruang': '11/matematika/vektor.html',
    'Transformasi Geometri': '11/matematika/transformasi-geometri.html',
    'Barisan & Deret Aritmetika': '11/matematika/barisan-aritmetika.html',
    'Barisan & Deret Geometri': '11/matematika/barisan-geometri.html',
    'Limit Fungsi Aljabar': '11/matematika/limit-fungsi.html',
  };
  const page = babPages[bab];
  if (page) {
    // Buka materi via iframe overlay — Firebase di app.html TIDAK mati
    openMateriOverlay(subj, bab, page);
  }
}

// ─────────────────────────────────────────────────────────
// MATERI IFRAME OVERLAY — buka/tutup tanpa pindah halaman
// ─────────────────────────────────────────────────────────
function openMateriOverlay(subj, bab, page) {
  const overlay = document.getElementById('materi-overlay');
  const iframe  = document.getElementById('materi-iframe');
  const title   = document.getElementById('materi-overlay-title');
  if (!overlay || !iframe) return;

  // Catat aktivitas & increment materiTotal SEKARANG (Firebase masih hidup)
  if (typeof FirebaseSync !== 'undefined') {
    FirebaseSync.onMateriOpen(subj, bab);
    // XP untuk buka materi
  ...
  // Dengarkan pesan "kembali" dari dalam iframe (postMessage)
  window._materiMessageHandler = function(e) {
    if (e.data === 'materi:goBack') {
      closeMateriOverlay();
    }
  };
  window.addEventListener('message', window._materiMessageHandler);
}

function closeMateriOverlay() {
  const overlay = document.getElementById('materi-overlay');
  const iframe  = document.getElementById('materi-iframe');
  if (!overlay) return;
  ...
  // Kosongkan iframe supaya tidak makan memori
  if (iframe) iframe.src = '';

  // Bersihkan message listener
  if (window._materiMessageHandler) {
    window.removeEventListener('message', window._materiMessageHandler);
    window._materiMessageHandler = null;
  }

  // Refresh stat widgets karena materiTotal sudah diupdate
  if (typeof renderAllStatWidgets === 'function') {
    setTimeout(renderAllStatWidgets, 200);
  }
  // Kembali ke panel materi
  if (typeof goPanel === 'function') goPanel('materi');
}
...
function selectBab(s, b) {
  ...
  modal.innerHTML = `
    <div style="background:#fff;border-radius:24px;padding:2rem 1.75rem 1.75rem;max-width:300px;width:100%;text-align:center;box-shadow:0 20px 60px rgba(0,0,0,.2);position:relative">
      <div style="font-size:3.2rem;margin-bottom:.75rem">🔒</div>
      <div style="font-size:1.2rem;font-weight:900;color:#1e1b4b;margin-bottom:.4rem">Segera Hadir!</div>
      <div style="font-size:.88rem;font-weight:700;color:#334155;margin-bottom:.3rem">${b}</div>
      <div style="font-size:.78rem;color:#64748b;margin-bottom:1.5rem;line-height:1.5">sedang dalam tahap pengembangan dan<br>akan segera tersedia.<br><br>Nantikan update berikutnya! 🚀</div>
      <button onclick="document.getElementById('coming-soon-modal').remove()" style="background:#3b82f6;color:#fff;border:none;border-radius:12px;padding:.7rem 0;font-weight:800;font-size:.92rem;cursor:pointer;width:100%">Oke, Mengerti!</button>
    </div>
  `;
...
function mascotTalk() {
  ...
  const svg   = document.getElementById('mascot-char');

  // Blink: squish ry → 0.3 lalu restore ke nilai rx asli (bulat sempurna)
  if (eyeL && eyeR) {
    const ryL = eyeL.getAttribute('rx') || '2';
    const ryR = eyeR.getAttribute('rx') || '2';
    eyeL.setAttribute('ry', '0.3');
    eyeR.setAttribute('ry', '0.3');
    setTimeout(() => {
      eyeL.setAttribute('ry', ryL);
      eyeR.setAttribute('ry', ryR);
    }, 150);
  }

  // Bounce: pause animasi bob, jalankan bounce, lalu resume
  if (svg) {
    svg.style.animationPlayState = 'paused';
    svg.style.transition = 'transform 0.1s cubic-bezier(.36,.07,.19,.97)';
    svg.style.transform  = 'scale(1.2) rotate(-8deg)';
    setTimeout(() => {
      svg.style.transform = 'scale(0.9) rotate(5deg)';
      setTimeout(() => {
...
// xp, coins, streak, accent, stats → selalu dari Firestore via FirebaseSync
// Session hanya untuk: username, role, character
// ===========================
(function initFromSession() {
  const session = (typeof Auth !== 'undefined') ? Auth.getSession() : null;
  if (!session) return;

  // Hanya ambil info auth dari session
  ...
  // Update UI dasar (nama, role) — angka xp/coins/streak diisi oleh FirebaseSync
...
(function initFlow() {
  ...
  if (savedChar) {
    ...
    goScreen('sc-main');
    document.getElementById('mobile-bottom-nav').classList.add('on');
    // Init Firestore realtime sync setelah masuk — DOM & semua fungsi sudah siap
...
const StreakSystem = (() => {
  ...
  function getData() {
    try { return (window._streakData && typeof window._streakData === 'object') ? window._streakData : { days: [], lastShown: null }; }
    catch { return { days: [], lastShown: null }; }
  }

  function saveData(d) {
    window._streakData = d;
    // Simpan ke Firestore — streak.days dan stats.streakCount
    // saveStreak sekarang LANGSUNG (non-debounced) agar tidak hilang saat navigasi
    if (typeof FirebaseSync !== 'undefined') {
      const count = calcStreak(d.days || []);
      // saveStreak sudah async-immediate — langsung tulis streak.days + lastShown
      FirebaseSync.saveStreak(d).then(() => {
        // Setelah streak tersimpan, update streakCount di stats LANGSUNG (tanpa debounce)
        FirebaseSync.saveStreakCount(count);
      }).catch(() => {
        // Fallback jika promise gagal
        FirebaseSync.saveStreakCount(count);
      });
    }
  }
  ...
  function checkAndShow() {
    ...
    async function _doCheck() {
      ...
      // RACE CONDITION GUARD: jika memori in-page (_streakData) lebih baru
      // dari Firestore (misal: baru saja login tapi navigasi terlalu cepat),
      // gunakan data memori agar streak tidak terpotong
      const memData = getData();
      if (memData && Array.isArray(memData.days) && memData.days.length > 0) {
        if (memData.lastShown === todayStr) {
          // Memori sudah check-in hari ini → prioritaskan memori
          data = memData;
        } else if (memData.days.length >= (data.days ? data.days.length : 0)) {
          // Memori punya data lebih lengkap → merge (ambil union kedua set tanggal)
          const allDays = Array.from(new Set([...(data.days || []), ...memData.days])).sort();
          data.days = allDays;
          if (!data.lastShown && memData.lastShown) data.lastShown = memData.lastShown;
        }
      }
  ...
  function applyFromFirestore(streakData) {
    window._streakData = streakData || { days: [], lastShown: null };
    updateAllStreakUI(calcStreak(window._streakData.days));
  }

  function getDataInMemory() {
    return window._streakData || { days: [], lastShown: null };
  }

  return { checkAndShow, calcStreak, getData, updateAllStreakUI, resetCheckFlag, applyFromFirestore, getDataInMemory };
})();
...
// ===========================
// Override goDash agar jalankan streak SETELAH FirebaseSync pull selesai
const _origGoDash2 = window.goDash;
window.goDash = function() {
  _origGoDash2 && _origGoDash2();
  // Streak check setelah FirebaseSync ready
  (function _waitStreak() {
    if (typeof FirebaseSync !== 'undefined' && !FirebaseSync.isReady()) {
      setTimeout(_waitStreak, 150);
    } else {
      setTimeout(() => {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (scMain && scMain.classList.contains('on')) {
    // Tunggu FirebaseSync pull selesai, BARU check streak
    (function _waitStreakDom() {
      if (typeof FirebaseSync !== 'undefined' && !FirebaseSync.isReady()) {
        setTimeout(_waitStreakDom, 150);
      } else {
        setTimeout(() => {
          if (typeof StreakSystem !== 'undefined') StreakSystem.checkAndShow();
        }, 300);
...
const AgendaSystem = (() => {
  ...
  function load() {
    // Data diisi dari Firestore via applyFromFirestore(). Tidak perlu baca localStorage.
    agendaDB = {};
  }
  function save() {
    if (typeof FirebaseSync !== 'undefined') {
      // Pakai saveAgendaImmediate agar langsung tersimpan, tidak lewat debounce
      if (typeof FirebaseSync.saveAgendaImmediate === 'function') {
        FirebaseSync.saveAgendaImmediate(agendaDB);
      } else {
        FirebaseSync.saveAgenda(agendaDB);
      }
    }
  }
  ...
  function init() {
    ...
    renderHomeAgenda();
    scheduleAllNotifs();
    setTimeout(() => { if (typeof AgendaKanban !== 'undefined') AgendaKanban.render(selectedDate); agHomeCalRender(); }, 50);
  }

  function buildRingSVG(events, strokeWidth) {
    if (!events.length) return '';
    const acColor = getComputedStyle(document.documentElement).getPropertyValue('--ac').trim() || '#3b82f6';
    return `<svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" style="position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);pointer-events:none;overflow:visible;z-index:2"><circle cx="16" cy="16" r="14" fill="none" stroke="${acColor}" stroke-width="${strokeWidth}" stroke-linecap="round"/></svg>`;
  }
  ...
  function renderCalendar() {
    ...
    for (let d = 1; d <= daysInMonth; d++) {
      ...
      const ring = buildRingSVG(events, 2.5);
      html += `<div class="${cls}" onclick="AgendaSystem.selectDate('${ds}')">${ring}${d}</div>`;
    }
    // next month padding
  ...
  function selectDate(ds) {
    ...
    if (typeof AgendaKanban !== 'undefined') AgendaKanban.render(selectedDate);
  }
  ...
  function renderDayEvents() {
    ...
    function buildDoneCard(ev) {
      const claimed = ev.claimed === true;
      const claimBtn = claimed
        ? `<span style="font-size:.68rem;background:#f1f5f9;color:#94a3b8;border:1px solid #e2e8f0;border-radius:99px;padding:.2rem .6rem;font-weight:700;flex-shrink:0;white-space:nowrap">✓ Diklaim</span>`
        : `<button onclick="event.stopPropagation();AgendaSystem.claimReward('${selectedDate}','${ev.id}')" style="font-size:.68rem;background:linear-gradient(135deg,#f59e0b,#f97316);color:#fff;border:none;border-radius:99px;padding:.22rem .65rem;font-weight:800;cursor:pointer;flex-shrink:0;white-space:nowrap;transition:opacity .15s" onmouseover="this.style.opacity='.8'" onmouseout="this.style.opacity='1'">🎁 Klaim</button>`;
      return `<div class="ag-event-item" style="opacity:${claimed ? '.5' : '1'}">
        <div class="ag-bar ${ev.color}"></div>
        <div style="flex:1;min-width:0">
          <div style="font-weight:700;font-size:.88rem;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;${claimed ? 'text-decoration:line-through;color:#94a3b8' : ''}">${ev.name}</div>
          <div style="font-size:.72rem;color:#94a3b8;margin-top:.1rem">⏰ ${ev.timeStart} – ${ev.timeEnd}${ev.note ? ' · ' + ev.note : ''}</div>
        </div>
        ${claimBtn}
      </div>`;
    }
    ...
    // Completed events section with claim reward
    if (doneEvents.length) {
      const unclaimedCount = doneEvents.filter(({ev}) => !ev.claimed).length;
      const rewardBadge = unclaimedCount > 0
        ? `<span style="font-size:.63rem;background:#fef3c7;color:#92400e;border:1px solid #fcd34d;border-radius:99px;padding:.06rem .5rem;font-weight:800">🎁 ${unclaimedCount} hadiah</span>`
        : '';
      html += `<div style="margin-top:.75rem;padding-top:.6rem;border-top:1.5px dashed #e2e8f0">
        <div style="display:flex;align-items:center;gap:.35rem;margin-bottom:.45rem">
          <span style="font-size:.68rem;font-weight:800;color:#94a3b8;letter-spacing:.04em">✅ SELESAI</span>
        ...
        <div>${doneEvents.map(({ ev }) => buildDoneCard(ev)).join('')}</div>
      </div>`;
    }
  ...
  function renderHomeAgenda() {
    ...
    events.forEach(ev => { if (cnt[ev.color] !== undefined) cnt[ev.color]++; });
    const badges = [
      cnt.red    ? `<span style="display:inline-flex;align-items:center;gap:.22rem;background:#fff5f5;border:1px solid #fecaca;color:#dc2626;border-radius:99px;padding:.1rem .45rem;font-size:.7rem;font-weight:800">&#128308; ${cnt.red} Penting</span>` : '',
      cnt.yellow ? `<span style="display:inline-flex;align-items:center;gap:.22rem;background:#fffbeb;border:1px solid #fde68a;color:#d97706;border-radius:99px;padding:.1rem .45rem;font-size:.7rem;font-weight:800">&#128993; ${cnt.yellow} Sedang</span>` : '',
      cnt.green  ? `<span style="display:inline-flex;align-items:center;gap:.22rem;background:#f0fdf4;border:1px solid #bbf7d0;color:#16a34a;border-radius:99px;padding:.1rem .45rem;font-size:.7rem;font-weight:800">&#128994; ${cnt.green} Santai</span>` : '',
    ].filter(Boolean).join('');
    list.innerHTML = `<div style="display:flex;gap:.35rem;flex-wrap:wrap;margin-bottom:.5rem">${badges}</div>`;
  }
  ...
  function openForm(dateStr, eventId) {
    ...
    // Reset semua tombol warna ke netral
    ...
    const colorStyles = {
      red:    { border: '#ef4444', bg: '#fee2e2', color: '#991b1b' },
      yellow: { border: '#eab308', bg: '#fef9c3', color: '#854d0e' },
      green:  { border: '#22c55e', bg: '#dcfce7', color: '#166634' }
    };
    ...
    if (eventId) {
      ...
      if (ev) {
        ...
        if (cb) {
          ...
          const s = colorStyles[ev.color];
          if (s) { cb.style.borderColor = s.border; cb.style.background = s.bg; cb.style.color = s.color; }
        }
      }
    } else {
  ...
  return {
    ...
    claimReward: (ds, id) => {
      ...
      if (typeof addCoins === 'function') addCoins(1);
      if (typeof SFX !== 'undefined') SFX.correct();
      renderDayEvents();
    },
    setColor: (c) => { selectedColor = c; },
    getSelectedDate: () => selectedDate,
    refresh: () => { renderCalendar(); renderDayEvents(); renderHomeAgenda(); if (typeof AgendaKanban !== 'undefined') AgendaKanban.render(selectedDate); setTimeout(agHomeCalRender, 20); },
    requestNotifPermission,
    applyFromFirestore: (data) => {
      agendaDB = (data && typeof data === 'object') ? data : {};
      renderCalendar();
      renderDayEvents();
      renderHomeAgenda();
      scheduleAllNotifs();
      setTimeout(agHomeCalRender, 20);
    },
    getDataInMemory: () => agendaDB,
    getCalInfo: () => ({
      year: currentYear,
      month: currentMonth,
      selectedDate,
...
function agHomeCalRender() {
  ...
  const colorPriority = { red: 0, yellow: 1, green: 2 }; // kept for sort order only
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    const ring = buildRingSVG(events, 2);
    html += `<div class="${cls}" style="font-size:.68rem;min-height:22px" onclick="AgendaSystem.selectDate('${ds}');agHomeCalRender();goPanel('agenda')">${ring}${d}</div>`;
  }
  const total = firstDay + daysInMonth;
  const rem = total % 7 === 0 ? 0 : 7 - (total % 7);
  for (let i = 1; i <= rem; i++) {
...
// statData is dynamically built from real in-memory activity logs
...
function renderStatBars(scale) {
  ...
  let xpEarned = 0;
  try {
    const xpHist = (Array.isArray(window._xpHistory) ? window._xpHistory : []);
...
const OnlineLeaderboard = (() => {
  ...
  function loadFromFirebase() {
    try {
      // Leaderboard dibaca dari koleksi 'leaderboard' (publik, denormalized)
      // agar tidak butuh izin 'list' pada koleksi 'users' yang terbatas untuk admin saja.
      if (typeof fbDB !== 'undefined') {
        fbDB.collection('leaderboard').orderBy('xp', 'desc').limit(20).get().then(snap => {
          realPlayers = [];
          snap.forEach(doc => {
            const d = doc.data();
            if (d && d.username) {
...
const AgendaSound = (() => {
  ...
  function check() {
    ...
    let agendaDB = {};
    // agendaDB sudah ada di memori AgendaSystem dari Firestore
    ...
    events.forEach(ev => {
      ...
      // [15-min reminder dihapus per request]
    });
  }
...
function renderXPChart(scale) {
  ...
  // Load XP history
  let logs = [];
  try { logs = (Array.isArray(window._xpHistory) ? window._xpHistory : []); } catch(e) {}
...
(function() {
  ...
  async function hostStartBattle(room, code) {
    ...
    try {
      ...
      const questions = soal.slice(0, 10).map(q => ({
        q: String(q.q),
        o: (Array.isArray(q.o) ? q.o : Object.values(q.o || {})).map(String),
        a: typeof q.a === 'number' ? q.a : parseInt(q.a) || 0,
      }));
  ...
  function enterBattleFromRoom(room) {
    ...
    // Firestore kadang menyimpan array nested sebagai object — pastikan selalu array
    window._btQs = (bs.questions || []).map(q => ({
      q: q.q,
      o: Array.isArray(q.o) ? q.o : Object.values(q.o || {}),
      a: typeof q.a === 'number' ? q.a : parseInt(q.a) || 0,
    }));
  ...
  function finishPrivateBattle(won, code) {
    ...
    if (won) { SFX.win(); addXP(xpGain); addCoins(5); }
    else { SFX.lose(); addXP(-xpLoss); }
    logActivity(window._btSubj || 'Fisika', Math.max(2, btQ), 'battle');
    // NOTE: onBattleResult dipanggil dari hook arenaStatRecord — JANGAN panggil lagi di sini
    if (typeof FirebaseSync !== 'undefined') {
      FirebaseSync.saveXPLevel(xp, level);
    }
  ...
  window.answerBattle = function(idx) {
    ...
    btAnswered = true; clearInterval(btInterval);
    // Pakai soal yang sudah di-shuffle agar indeks jawaban benar sesuai tampilan
    const q = window._btCurrentQ || window._btQs[btQ];
    const btns = document.querySelectorAll('#bt-opts button');
    btns.forEach(b => b.style.cursor = 'default');
    ...
    const unsub = fbDB.collection('rooms').doc(code).onSnapshot((snap) => {
      ...
      if (ended.startsWith(opponentUid)) {
        ...
        if (!window._btFinished) {
          ...
          SFX.win(); addXP(btXpWin); addCoins(5);
          logActivity(window._btSubj || 'Fisika', Math.max(2, btQ), 'battle');
          // NOTE: onBattleResult dipanggil dari hook arenaStatRecord — JANGAN panggil lagi di sini
          if (typeof FirebaseSync !== 'undefined') { FirebaseSync.saveXPLevel(xp, level); }
          setTimeout(() => showArenaMode('sel'), 3000);
...
// ── Storage helpers (in-memory, diisi dari Firestore) ──
window._statArena  = window._statArena  || null;
window._statMateri = window._statMateri || null;
window._statPomo   = window._statPomo   || null;

function _statGet(key) {
  if (key === 'arena')  return window._statArena  || null;
  if (key === 'materi') return window._statMateri || null;
  if (key === 'pomo')   return window._statPomo   || null;
  return null;
}
function _statSet(key, val) {
  if (key === 'arena')  window._statArena  = val;
  if (key === 'materi') window._statMateri = val;
  if (key === 'pomo')   window._statPomo   = val;
}
...
function renderArenaStatWidget() {
  ...
  const elLastTime  = document.getElementById('arena-stat-last-time');
  const elLastBadge = document.getElementById('arena-stat-last-badge');

  if (elTotal) elTotal.textContent = total;

  if (d.lastTs) {
    const ago = _timeAgo(d.lastTs);
    const mapelLabel = d.lastMapel || 'Arena';
    if (elLast)      elLast.textContent      = mapelLabel;
    if (elLastTime)  elLastTime.textContent  = '⚔️ Battle terakhir: ' + ago;
    if (elLastBadge) elLastBadge.textContent = ago;
  } else {
    if (elLast)      elLast.textContent      = '—';
    if (elLastTime)  elLastTime.textContent  = 'Belum pernah battle';
    if (elLastBadge) elLastBadge.textContent = '—';
  }
}
...
function renderMateriStatWidget() {
  const d = _statGet('materi') || { total: 0, lastMapel: null, lastBab: null, lastTs: null, lastOnlineTs: null };
  const total = d.total || 0;

  const _el = id => document.getElementById(id);
  const _t  = (id, v) => { const e = _el(id); if (e) e.textContent = v; };

  _t('materi-stat-total', total);

  const mapelShort = { matematika:'Mat', fisika:'Fisika', kimia:'Kimia', biologi:'Bio',
    bahasa_inggris:'B.Ing', bahasa_indonesia:'B.Ind', sejarah:'Sej', Flashcard:'Flash' };

  // Tampilan "terakhir belajar": aktivitas belajar atau waktu login terakhir
  const lastActivity = d.lastTs || d.lastOnlineTs || null;
  const badgeEl = _el('materi-stat-last-badge');
  if (badgeEl) badgeEl.textContent = lastActivity ? _timeAgo(lastActivity) : '—';

  if (total > 0) {
    const mapelLabel = mapelShort[d.lastMapel] || d.lastMapel || 'Belajar';
    _t('materi-stat-mapel', mapelLabel);
    const lanjutEl = _el('materi-lanjut-area');
    const emptyEl  = _el('materi-empty-area');
    if (lanjutEl) lanjutEl.style.display = d.lastMapel ? '' : 'none';
    if (emptyEl)  emptyEl.style.display  = 'none';
    const lanjutNm = _el('materi-lanjut-nama');
    if (lanjutNm) lanjutNm.textContent = (d.lastBab || d.lastMapel || 'Materi') + (lastActivity ? ' · ' + _timeAgo(lastActivity) : '');
    _lastMateriContext = { mapel: d.lastMapel, bab: d.lastBab };
  } else {
    _t('materi-stat-mapel', lastActivity ? _timeAgo(lastActivity) : '—');
    const lanjutEl = _el('materi-lanjut-area');
    const emptyEl  = _el('materi-empty-area');
    if (lanjutEl) lanjutEl.style.display = 'none';
    // Tampilkan "terakhir online" jika belum ada sesi tapi sudah pernah login
    if (emptyEl) {
      emptyEl.style.display = '';
      emptyEl.textContent = lastActivity
        ? '🕐 Terakhir online: ' + _timeAgo(lastActivity) + '. Mulai belajar!'
        : '📚 Belum ada sesi belajar. Mulai dari Materi Belajar!';
    }
...
// ── Render statistik di Settings Modal ──
function renderSettingsStats() {
  // Materi
  const m = _statGet('materi') || { total: 0, lastMapel: null, lastBab: null, lastTs: null };
  ...
  _s('st-materi-sesi', m.total || 0);
  _s('st-materi-last', m.lastMapel ? (mapelShort[m.lastMapel] || m.lastMapel) : '—');
  _s('st-materi-time', m.lastTs ? '🕐 ' + _timeAgo(m.lastTs) + (m.lastBab ? ' · ' + m.lastBab : '') : 'Belum ada sesi belajar');

  // Arena
  const a = _statGet('arena') || { total: 0, wins: 0, lastTs: null, lastMapel: null };
  _s('st-arena-sesi', a.total || 0);
  _s('st-arena-last', a.lastMapel || '—');
  _s('st-arena-time', a.lastTs ? '🕐 ' + _timeAgo(a.lastTs) : 'Belum pernah battle');

  // Pomodoro
  const today = new Date().toDateString();
  const p = _statGet('pomo') || { total: 0, todayDate: null, todayCount: 0, lastTs: null, lastMapel: null };
  const todayCount = p.todayDate === today ? (p.todayCount || 0) : 0;
  _s('st-pomo-sesi', todayCount);
  _s('st-pomo-last', p.lastMapel || '—');
  _s('st-pomo-time', p.lastTs ? '🕐 ' + _timeAgo(p.lastTs) : 'Belum ada sesi fokus');
}

// ── Pomodoro stat tracking ──
function pomoStatRecord(durationMinutes) {
  const today = new Date().toDateString();
  const d = _statGet('pomo') || { total: 0, todayDate: null, todayCount: 0, todayMnt: 0, lastTs: null, lastMapel: null };
  ...
  d.lastTs    = Date.now();
  d.lastMapel = (typeof currentPomoMapel !== 'undefined' && currentPomoMapel) ? currentPomoMapel : (window._lastPomoMapel || null);
  _statSet('pomo', d);
  renderPomoStatWidget();
}
...
function renderPomoStatWidget() {
  ...
  const elLast      = document.getElementById('pomo-stat-last');
  const elMnt       = document.getElementById('pomo-stat-mnt');
  const elLastBadge = document.getElementById('pomo-stat-last-badge');

  if (elHari) elHari.textContent = todayCount;

  if (d.lastTs) {
    const ago = _timeAgo(d.lastTs);
    const label = d.lastMapel || 'Fokus';
    if (elLast)      elLast.textContent      = label;
    if (elLastBadge) elLastBadge.textContent = ago;
  } else {
    if (elLast)      elLast.textContent      = '—';
    if (elLastBadge) elLastBadge.textContent = '—';
  }
  if (elMnt) elMnt.textContent = todayMnt > 0 ? todayMnt + ' menit fokus hari ini' : '0 menit fokus hari ini';
}
...
(function() {
  ...
  window.selectBab = function(subj, bab) {
    materiStatRecord(subj, bab);
    if (_origSB) _origSB.apply(this, arguments); // fix: pakai _origSB bukan _orig
  };
})();

// Hook pomodoro selesai sesi → pomo stat
// pSessions++ sudah ada di baris ~661, kita hook dari logActivity type 'pomo'
// logActivity hook — TIDAK lagi memanggil stat functions
// karena openBab, selectBab, dan pomodoro completion sudah di-hook secara langsung
// Memanggil lagi dari sini akan menyebabkan DOUBLE COUNT
// (function() { ... }); // dihapus
...
// Stat widgets akan dirender oleh FirebaseSync setelah data Firestore masuk
// FirebaseSync._applyStatsToApp → renderAllStatWidgets()
// Render ulang saat buka panel home (data dari Firestore sudah ada di memori)
(function() {
  const _origGoPanel = window.goPanel;
  window.goPanel = function(panel) {
    if (_origGoPanel) _origGoPanel.apply(this, arguments);
    if (panel === 'home') {
      // Hanya render jika FirebaseSync sudah ready (data Firestore sudah masuk)
      if (typeof FirebaseSync === 'undefined' || FirebaseSync.isReady()) {
        setTimeout(renderAllStatWidgets, 100);
      }
    }
  };
})();
...
// ═══════════════════════════════════════════════════════════════
// STAT WIDGETS — Write ke Firestore setiap kali ada aksi
// Read dari Firestore sudah dihandle oleh FirebaseSync.init()
// ═══════════════════════════════════════════════════════════════
(function() {

  // Override arenaStatRecord → update memori + push ke Firestore
  const _origArena = window.arenaStatRecord;
  window.arenaStatRecord = function(won) {
    if (_origArena) _origArena(won); // update _statArena memori
    // Push ke Firestore (single source — finishBattle TIDAK perlu panggil sendiri)
    if (typeof FirebaseSync !== 'undefined' && FirebaseSync.onBattleResult) {
      FirebaseSync.onBattleResult(won, won ? (typeof btXpWin !== 'undefined' ? btXpWin : 50) : 0);
    }
    console.log('[Stat] arenaStatRecord called, won:', won);
  };

  // Override materiStatRecord → update memori saja
  // FirebaseSync.onMateriOpen sudah dipanggil langsung di openMateriOverlay()
  // JANGAN panggil lagi di sini — akan menyebabkan double increment di Firestore
  const _origMateri = window.materiStatRecord;
  window.materiStatRecord = function(mapel, bab) {
    if (_origMateri) _origMateri(mapel, bab); // update _statMateri memori
    console.log('[Stat] materiStatRecord called:', mapel, bab);
  };

  // pomoStatRecord update memori (untuk tampilan widget)
  // FirebaseSync.onPomodoroComplete() sudah dipanggil langsung di timer completion (line ~697)
  // TIDAK perlu dipanggil lagi di sini — akan menyebabkan double write ke Firestore
  const _origPomo = window.pomoStatRecord;
  window.pomoStatRecord = function(durationMinutes) {
    if (_origPomo) _origPomo(durationMinutes); // update _statPomo memori
    // FirebaseSync.onPomodoroComplete() sudah dipanggil di timer — JANGAN panggil lagi
  };

})();

// ═══════════════════════════════════════════════════════════════
// BEFORE UNLOAD — Flush semua data ke Firestore sebelum pindah halaman
// Mencegah data hilang akibat debounce yang tercancel saat navigasi
// ═══════════════════════════════════════════════════════════════
window.addEventListener('pagehide', function() {
  if (typeof FirebaseSync !== 'undefined' && FirebaseSync.flushAll) {
    FirebaseSync.flushAll();
  }
});
...
const SHOP_ITEMS = {
  skin: [
    { id:'skin-1', name:'Default',        price:0,   free:true },
    { id:'skin-2', name:'Default Tan',    price:10  },
    { id:'skin-3', name:'Kucing Abu',     price:25  },
    { id:'skin-4', name:'Kucing Oren',    price:25  },

  ],
  acc: [
    { id:'acc-1', name:'Bandana',         price:10  },
    { id:'acc-2', name:'Kacamata',        price:10  },
    { id:'acc-3', name:'Pita Abu',        price:15  },
    { id:'acc-4', name:'Pita Oren',       price:15  },
    { id:'acc-5', name:'Kalung Fox Abu',  price:20  },
    { id:'acc-6', name:'Kalung Fox Oren', price:20  },
  ]
};

// ── Shop state (in-memory, diisi dari Firestore) ──
window._shopOwned      = window._shopOwned      || ['skin-1'];
window._shopActiveSkin = window._shopActiveSkin || 'skin-1';
window._shopActiveAcc  = window._shopActiveAcc  || '';

function getOwnedItems() {
  return Array.isArray(window._shopOwned) ? window._shopOwned : ['skin-1'];
}
function getActiveItem(cat) {
  if (cat === 'skin') return window._shopActiveSkin || 'skin-1';
  return window._shopActiveAcc || '';
}
function saveOwned(arr) {
  window._shopOwned = arr;
  _saveShopToFirestore();
}
function _saveShopToFirestore() {
  if (typeof FirebaseSync !== 'undefined') {
    const shopData = {
      owned:      Array.isArray(window._shopOwned) ? window._shopOwned : ['skin-1'],
      activeSkin: window._shopActiveSkin || 'skin-1',
      activeAcc:  window._shopActiveAcc  || '',
    };
    // Pakai saveShopImmediate agar langsung tersimpan, tidak lewat debounce
    if (typeof FirebaseSync.saveShopImmediate === 'function') {
      FirebaseSync.saveShopImmediate(shopData);
    } else {
      FirebaseSync.saveShop(shopData);
    }
  }
}
...
function renderShopButtons() {
  ...
  ['skin','acc'].forEach(cat => {
    SHOP_ITEMS[cat].forEach((item, idx) => {
      const slot = document.getElementById('shop-btn-' + (cat === 'acc' ? 'acc' : 'skin') + '-' + (idx+1));
      if (!slot) return;
      const isOwned  = owned.includes(item.id) || item.free;
      const isActive = getActiveItem(cat) === item.id;
      if (isActive) {
        slot.innerHTML = `<div class="shop-active-btn">✅ Aktif</div>`;
      } else if (isOwned) {
        slot.innerHTML = `<button class="shop-buy-btn" style="background:#3b82f6" onclick="activateShopItem('${cat}','${item.id}')">🎯 Pakai</button>`;
      } else {
        slot.innerHTML = `<button class="shop-buy-btn" onclick="buyShopItem('${cat}','${item.id}',${item.price})">
          <svg width="11" height="11" viewBox="0 0 24 24" fill="#fbbf24"><circle cx="12" cy="12" r="10"/><circle cx="12" cy="12" r="6" fill="#fde047"/></svg>
          ${item.price} Koin
        </button>`;
      }
    });
  });
  const coinEl = document.getElementById('shop-coin-display');
  if (coinEl) coinEl.textContent = coins;
}

function buyShopItem(cat, itemId, price) {
  if (coins < price) { toast('❌ Koin tidak cukup! Butuh ' + price + ' koin.'); return; }
  coins -= price;
  if (typeof FirebaseSync !== 'undefined') FirebaseSync.saveCoins(coins);
  updateCoinUI();
  const owned = getOwnedItems();
  if (!owned.includes(itemId)) { owned.push(itemId); saveOwned(owned); }
  activateShopItem(cat, itemId);
  toast('🎉 Item berhasil dibeli & dipakai!');
}

function activateShopItem(cat, itemId) {
  if (cat === 'skin') window._shopActiveSkin = itemId;
  else window._shopActiveAcc = itemId;
  _saveShopToFirestore(); // langsung save tanpa debounce
  applyMascotSkin();
  renderShopButtons();
}

function applyMascotSkin() {
  const mascotSvg = document.getElementById('mascot-char');
  if (!mascotSvg) return;
  const skin = getActiveItem('skin');
  const acc  = getActiveItem('acc');

  const skins = {
    'skin-1': `<ellipse cx="40" cy="52" rx="18" ry="22" fill="#3b82f6"/><circle cx="40" cy="28" r="18" fill="#FDDBB4"/><circle cx="34" cy="26" r="3.5" fill="#fff"/><circle cx="46" cy="26" r="3.5" fill="#fff"/><ellipse cx="35" cy="27" rx="2" ry="2" fill="#1e293b" id="m-eye-l"/><ellipse cx="47" cy="27" rx="2" ry="2" fill="#1e293b" id="m-eye-r"/><circle cx="35.8" cy="26.2" r=".7" fill="#fff"/><circle cx="47.8" cy="26.2" r=".7" fill="#fff"/><path id="m-mouth" d="M34,34 Q40,39 46,34" stroke="#b45309" stroke-width="2" fill="none" stroke-linecap="round"/><ellipse cx="28" cy="31" rx="5" ry="3" fill="#fca5a5" opacity=".5"/><ellipse cx="52" cy="31" rx="5" ry="3" fill="#fca5a5" opacity=".5"/><ellipse cx="22" cy="53" rx="5" ry="11" fill="#FDDBB4" transform="rotate(-15 22 53)"/><ellipse cx="58" cy="53" rx="5" ry="11" fill="#FDDBB4" transform="rotate(15 58 53)"/><rect x="26" y="62" width="28" height="18" rx="4" fill="#fff" stroke="#3b82f6" stroke-width="1.5"/><line x1="40" y1="63" x2="40" y2="79" stroke="#3b82f6" stroke-width="1.5"/><rect x="24" y="12" width="32" height="5" rx="2.5" fill="#1e40af"/><rect x="30" y="6" width="20" height="10" rx="3" fill="#1e40af"/><rect x="36" y="5" width="8" height="3" rx="1.5" fill="#fbbf24"/>`,
    'skin-2': `<ellipse cx="40" cy="52" rx="18" ry="22" fill="#7c3aed"/><circle cx="40" cy="28" r="18" fill="#C68642"/><circle cx="34" cy="26" r="3.5" fill="#fff"/><circle cx="46" cy="26" r="3.5" fill="#fff"/><ellipse cx="35" cy="27" rx="2" ry="2" fill="#1e293b" id="m-eye-l"/><ellipse cx="47" cy="27" rx="2" ry="2" fill="#1e293b" id="m-eye-r"/><path id="m-mouth" d="M34,34 Q40,39 46,34" stroke="#92400e" stroke-width="2" fill="none" stroke-linecap="round"/><ellipse cx="22" cy="53" rx="5" ry="11" fill="#C68642" transform="rotate(-15 22 53)"/><ellipse cx="58" cy="53" rx="5" ry="11" fill="#C68642" transform="rotate(15 58 53)"/><rect x="26" y="62" width="28" height="18" rx="4" fill="#fff" stroke="#7c3aed" stroke-width="1.5"/><line x1="40" y1="63" x2="40" y2="79" stroke="#7c3aed" stroke-width="1.5"/><rect x="24" y="12" width="32" height="5" rx="2.5" fill="#5b21b6"/><rect x="30" y="6" width="20" height="10" rx="3" fill="#5b21b6"/>`,
    'skin-3': `<path d="M20,38 L26,18 L36,38 Z" fill="#c8cdd2"/><path d="M52,38 L62,18 L68,38 Z" fill="#c8cdd2"/><circle cx="44" cy="46" r="26" fill="#c8cdd2"/><ellipse cx="44" cy="53" rx="13" ry="9" fill="#f0f0f0"/><circle cx="35" cy="44" r="7" fill="#fff"/><ellipse cx="35" cy="44" rx="3.5" ry="3.5" fill="#111" id="m-eye-l"/><circle cx="33.5" cy="42.5" r="1.4" fill="#fff"/><circle cx="53" cy="44" r="7" fill="#fff"/><ellipse cx="53" cy="44" rx="3.5" ry="3.5" fill="#111" id="m-eye-r"/><circle cx="51.5" cy="42.5" r="1.4" fill="#fff"/><path d="M42,52 L44,54.5 L46,52 Q44,51 42,52 Z" fill="#e88fa0"/><path id="m-mouth" d="M40,55 Q44,58.5 48,55" stroke="#2d2d2d" stroke-width="1.5" fill="none"/><line x1="28" y1="51" x2="41" y2="53" stroke="#999" stroke-width="1" opacity=".7"/><line x1="28" y1="54" x2="41" y2="54" stroke="#999" stroke-width="1" opacity=".7"/><line x1="28" y1="57" x2="41" y2="55" stroke="#999" stroke-width="1" opacity=".7"/><line x1="60" y1="51" x2="47" y2="53" stroke="#999" stroke-width="1" opacity=".7"/><line x1="60" y1="54" x2="47" y2="54" stroke="#999" stroke-width="1" opacity=".7"/><line x1="60" y1="57" x2="47" y2="55" stroke="#999" stroke-width="1" opacity=".7"/><ellipse cx="44" cy="84" rx="28" ry="22" fill="#c8cdd2"/><ellipse cx="44" cy="86" rx="18" ry="14" fill="#f0f0f0"/><path d="M68,90 Q88,75 82,55 Q85,60 80,70 Q86,65 76,82 Z" fill="#b0b8c1"/>`,
    'skin-4': `<path d="M20,38 L26,18 L36,38 Z" fill="#e8a04a"/><path d="M52,38 L62,18 L68,38 Z" fill="#e8a04a"/><circle cx="44" cy="46" r="26" fill="#e8a04a"/><ellipse cx="44" cy="53" rx="13" ry="9" fill="#fef3c7"/><circle cx="35" cy="44" r="7" fill="#fff"/><ellipse cx="35" cy="44" rx="3.5" ry="3.5" fill="#111" id="m-eye-l"/><circle cx="33.5" cy="42.5" r="1.4" fill="#fff"/><circle cx="53" cy="44" r="7" fill="#fff"/><ellipse cx="53" cy="44" rx="3.5" ry="3.5" fill="#111" id="m-eye-r"/><circle cx="51.5" cy="42.5" r="1.4" fill="#fff"/><path d="M42,52 L44,54.5 L46,52 Q44,51 42,52 Z" fill="#e88fa0"/><path id="m-mouth" d="M40,55 Q44,58.5 48,55" stroke="#2d2d2d" stroke-width="1.5" fill="none"/><line x1="28" y1="51" x2="41" y2="53" stroke="#92400e" stroke-width="1" opacity=".6"/><line x1="28" y1="54" x2="41" y2="54" stroke="#92400e" stroke-width="1" opacity=".6"/><line x1="28" y1="57" x2="41" y2="55" stroke="#92400e" stroke-width="1" opacity=".6"/><line x1="60" y1="51" x2="47" y2="53" stroke="#92400e" stroke-width="1" opacity=".6"/><line x1="60" y1="54" x2="47" y2="54" stroke="#92400e" stroke-width="1" opacity=".6"/><line x1="60" y1="57" x2="47" y2="55" stroke="#92400e" stroke-width="1" opacity=".6"/><ellipse cx="44" cy="84" rx="28" ry="22" fill="#e8a04a"/><ellipse cx="44" cy="86" rx="18" ry="14" fill="#fef3c7"/><path d="M68,90 Q88,75 82,55 Q85,60 80,70 Q86,65 76,82 Z" fill="#d97706"/>`,
    
  };

  const accOverlays = {
    'acc-1': `<rect x="28" y="16" width="24" height="5" rx="2.5" fill="#ef4444"/><circle cx="40" cy="16" r="3" fill="#dc2626"/>`,
    'acc-2': `<rect x="28" y="24" width="10" height="7" rx="3" fill="none" stroke="#1e293b" stroke-width="2"/><rect x="42" y="24" width="10" height="7" rx="3" fill="none" stroke="#1e293b" stroke-width="2"/><line x1="38" y1="27.5" x2="42" y2="27.5" stroke="#1e293b" stroke-width="2"/>`,
    'acc-3': `<path d="M22,22 Q28,14 34,22" fill="#a3a3a3" opacity=".8"/><circle cx="28" cy="15" r="4" fill="#d1d5db" stroke="#9ca3af" stroke-width="1.5"/>`,
    'acc-4': `<path d="M22,22 Q28,14 34,22" fill="#f97316" opacity=".8"/><circle cx="28" cy="15" r="4" fill="#fed7aa" stroke="#f97316" stroke-width="1.5"/>`,
    'acc-5': `<ellipse cx="40" cy="51" rx="10" ry="2.5" fill="none" stroke="#6b7280" stroke-width="1.8"/><circle cx="40" cy="48.5" r="3" fill="#e5e7eb" stroke="#6b7280" stroke-width="1.2"/>`,
    'acc-6': `<ellipse cx="40" cy="51" rx="10" ry="2.5" fill="none" stroke="#ea580c" stroke-width="1.8"/><circle cx="40" cy="48.5" r="3" fill="#fef9c3" stroke="#ea580c" stroke-width="1.2"/>`
  };

  mascotSvg.innerHTML = (skins[skin] || skins['skin-1']) + ((acc && accOverlays[acc]) ? accOverlays[acc] : '');
}

function switchShopTab(tab, btn) {
  ['skin','acc'].forEach(t => {
    const el = document.getElementById('shop-panel-' + t);
    if (el) el.style.display = t === tab ? '' : 'none';
  });
  document.querySelectorAll('.shop-tab').forEach(b => b.classList.remove('act'));
  btn.classList.add('act');
  renderShopButtons();
}

// Patch openModal to render shop buttons
const _origOpenModal = window.openModal;
window.openModal = function(id) {
  if (typeof _origOpenModal === 'function') _origOpenModal(id);
  if (id === 'maskot-shop-modal') setTimeout(renderShopButtons, 50);
};

// Init shop on load
document.addEventListener('DOMContentLoaded', () => {
  const owned = getOwnedItems();
  if (!owned.includes('skin-1')) { owned.push('skin-1'); saveOwned(owned); }
  setTimeout(applyMascotSkin, 500);
});
