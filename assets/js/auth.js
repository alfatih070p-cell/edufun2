...
const Auth = (() => {
  ...
  async function register(username, password, role) {
    ...
    if (!password || password.length < 4) return { ok: false, msg: 'Password minimal 4 karakter.' };
    ...
    } catch { return { ok: false, msg: 'Gagal cek username. Coba lagi.' }; }
    ...
    try {
      ...
      await fbDB.collection('usernames').doc(username.toLowerCase()).set({ uid });
  ...
  return { register, login, logout, getSession, requireAuth, saveCharacter, getCharacter, updateStats };
})();
