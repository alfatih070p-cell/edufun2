...
const Auth = (() => {
  ...
  function _clearSession() {
    sessionStorage.removeItem(SESSION_KEY); // bersihkan sisa lama
  ...
  async function register(username, password, role) {
    ...
    // Cek username availability langsung dari Firestore (force server, skip cache)
    try {
      ...
      // Kalau offline atau error lain — coba lanjut, Firebase Auth yang jadi gatekeeper
    ...
    try {
      ...
      // Tulis data user ke Firestore
      await fbDB.collection('users').doc(uid).set(userData);
      // Buat userdata doc kosong (stats, dll akan diisi saat pakai app)
      await fbDB.collection('userdata').doc(uid).set({
        accent: '#3b82f6',
        coins: 0,
        shop: { owned: ['skin-1'], activeSkin: 'skin-1', activeAcc: '' },
        stats: {
          xp: 0, level: 1, streakCount: 0,
          battleTotal: 0, battleWins: 0, battleXP: 0, battleXP: 0,
          materiTotal: 0,
          pomodoroTotal: 0, pomodoroToday: 0, pomodoroTodayMnt: 0,
        },
        cards: [], agenda: {}, streak: { days: [], lastShown: null },
        xpHistory: [], activityLogs: [],
        createdAt: now,
      });
      // Simpan username mapping
      try { await fbDB.collection('usernames').doc(username.toLowerCase()).set({ uid, createdAt: now }); }
  ...
  async function login(username, password, rememberMe = false) {
    ...
    try {
      ...
      // Simpan lastOnline untuk widget "terakhir belajar" di beranda
      try {
        await fbDB.collection('userdata').doc(uid).set({ stats: { lastOnlineTs: now } }, { merge: true });
      } catch(e) {}
      const session = _buildSession(uid, { ...data, streak: newStreak, lastLogin: now }, now);
      // Selalu simpan ke localStorage agar session bertahan setelah tab ditutup
      // rememberMe hanya mengontrol TTL (7 hari vs 1 hari)
  ...
  // ---- Logout ----
  // Hapus semua data user dari localStorage saat logout
  // (tidak ada state sensitif di localStorage lagi — semua di Firestore)
  function clearUserStorage() {
    // Bersihkan session
    localStorage.removeItem('edufun_session');
    sessionStorage.removeItem('edufun_session');
    // Bersihkan sisa cache lama jika ada
    Object.keys(localStorage)
      .filter(k => k.startsWith('ef_') || k.startsWith('edufun_'))
      .forEach(k => localStorage.removeItem(k));
    // Reset variabel memori global
  ...
  async function logout() {
    // Reset streak check flag supaya saat login berikutnya tidak terblokir
